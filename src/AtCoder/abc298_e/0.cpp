...
ostream &operator<<(ostream &os, modint998244353 const &n) {
  return os << n.val();
}
auto solve([[maybe_unused]] int t_ = 0) -> void {
  int n, a, b, p, q;
  cin >> n >> a >> b >> p >> q;
  vvc<modint998244353> fp(n + 1, vc<modint998244353>(n + 1)),
    fq(n + 1, vc<modint998244353>(n + 1));
  fp[0][a] = fq[0][b] = 1;
  for_(i, 0, n - a - 1)
    for_(l, a + i, min(n - 1, a + i * p)) {
      modint998244353 _ = fp[i][l] / p;
      for_(j, l + 1, min(n, l + p)) fp[i + 1][j] += _;
      if (l + p > n) fp[i + 1][n] += (l + p - n) * _;
    }
  for_(i, 0, n - b - 1)
    for_(l, b + i, min(n - 1, b + i * q)) {
      modint998244353 _ = fq[i][l] / q;
      for_(j, l + 1, min(n, l + q)) fq[i + 1][j] += _;
      if (l + q > n) fq[i + 1][n] += (l + q - n) * _;
    }
  modint998244353 ans = 0, _ = 0;
  for_(i, n - a + 1, n - b) _ += fq[i][n];
  rfor_(i, n - a, 0) ans += fp[i][n] * (_ += fq[i][n]);
  cout << ans;
}
int main() {
