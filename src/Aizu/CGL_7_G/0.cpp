...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    vc<Line2> exl = extan_CC(c1, c2), inl = intan_CC(c1, c2);
    vc<Point2> ans;
    if (exl.size())
        foreach_cref_(l, exl) ans.push_back(l.l);
    if (inl.size())
        foreach_cref_(l, inl) ans.push_back(l.l);
    if (ans.size() == 0) return;
    sort(all_(ans));
    ans.erase(unique(all_(ans)), ans.end());
    foreach_cref_(p, ans) cout << p << '\n';
}
int main() {
