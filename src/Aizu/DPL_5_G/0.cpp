...
namespace tifa_libs::math {
vec<u64> pows_mod_gen(u64 sz, u64 n, u64 mod) {
  vec<u64> ans(sz);
  if (!sz) return {};
  if (!n) ans[0] = 1;
  if (sz == 1) return ans;
...
namespace tifa_libs::math {
template <class T>
poly<T> stirling2_row_gen(u64 n, vec<u64> const &pows, vec<u64> const &ifact) {
...
template <class T>
poly<T> stirling2_row_gen(u64 n) {
  return stirling2_row_gen<T>(n,
                              pows_mod_gen(n + 1, n, T::value_type::mod()),
                              ifact_mod_gen(n + 1, T::value_type::mod()));
}
}  // namespace tifa_libs::math
namespace tifa_libs::math {
template <class T>
constexpr auto ball_box_dia(u64 ball, u64 box, poly<T> const &s2r) {
  return std::reduce(
    s2r.data().begin(),
    std::next(s2r.data().begin(), (isz)std::min(ball, box) + 1),
    typename poly<T>::value_type{});
}
template <class T>
constexpr auto ball_box_dia(u64 ball, u64 box) {
  return ball_box_dia<T>(ball, box, stirling2_row_gen<T>(ball));
}
}  // namespace tifa_libs::math
namespace tifa_libs::math {
template <int>
