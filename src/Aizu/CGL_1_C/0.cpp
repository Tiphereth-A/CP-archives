...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    for_(i, 1, q) {
        cin >> p;
        if (is_positive(cross(p0, p1, p))) run_continue_(cout << "COUNTER_CLOCKWISE\n");
        if (is_negative(cross(p0, p1, p))) run_continue_(cout << "CLOCKWISE\n");
        if (is_negative(dot(p0, p1, p))) run_continue_(cout << "ONLINE_BACK\n");
        if (is_in_middle(p0, p, p1)) run_continue_(cout << "ON_SEGMENT\n");
        run_continue_(cout << "ONLINE_FRONT\n");
    }
}
int main() {
