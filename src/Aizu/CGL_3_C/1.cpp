...
template <class FP>
constexpr bool
is_on_same_L(point<FP> const &p1, point<FP> const &p2, point<FP> const &p3) {
...
namespace tifa_libs::geo {
template <class FP>
constexpr bool is_on_S(line<FP> const &s, point<FP> const &p) {
  return is_on_same_L(s.l, s.r, p) && is_in_middle(s.l, p, s.r);
}
template <class FP>
constexpr bool is_on_S_strict(line<FP> const &s, point<FP> const &p) {
  return sgn_cross(s.l, s.r, p) == 0 &&
         sgn((p - s.l) * (s.l - s.r)) * sgn((p - s.r) * (s.l - s.r)) < 0;
}
}  // namespace tifa_libs::geo
namespace tifa_libs::geo {
enum RELPoP { outside_pop, onborder_pop, onendpoint_pop, inside_pop };
template <class FP>
...
RELPoP relation_CvhP(cvh<FP> const &cvh, point<FP> const &p) {
  ...
  if (is_pos(cross(cvh.vs.front(), p, cvh.vs[1])) ||
      is_pos(cross(cvh.vs.front(), cvh.vs.back(), p)))
    return outside_pop;
  auto it = std::lower_bound(cvh.vs.begin() + 1,
                             cvh.vs.end(),
                             p,
                             [&](point<FP> const &lhs, point<FP> const &rhs) {
                               return is_pos(cross(cvh.vs.front(), lhs, rhs));
                             }) -
            1;
