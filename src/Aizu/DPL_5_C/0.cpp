...
namespace tifa_libs::math {
template <class mint>
mint ball_box_ddl(u64, u64 box, vec<u64> const &pows, Binom const &binom) {
  mint ans = 0;
  u64 f = box & 1;
  for (u64 i = 1; i <= box; ++i) {
    f ? (ans += binom.mCn(box, i) * pows[i]) :
        (ans -= binom.mCn(box, i) * pows[i]);
    f ^= 1;
  }
  return ans;
}
template <class mint>
mint ball_box_ddl(u64 ball, u64 box, Binom const &binom) {
  return ball_box_ddl<mint>(
    ball, box, pows_mod_gen(box + 1, ball, mint::mod()), binom);
}
}  // namespace tifa_libs::math
namespace tifa_libs::math {
template <u32 MOD>
