...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  read_var_(int, n);
  read_container_(vc<pii>, a, n);
  sort(all_(a));
  vector<int> mx(n + 1);
  mx[n] = INT_MIN;
  rfor_(i, n - 1, 0) mx[i] = max(mx[i + 1], a[i].second);
  pq<int> pq_le;
  pqg<int> pq_gt;
  int ans = INT_MAX;
  for_(i, 0, n - 1) {
    int p = a[i].first;
    while (!pq_gt.empty() && pq_gt.top() <= p) {
      pq_le.push(pq_gt.top());
      pq_gt.pop();
    }
    int srt = INT_MAX;
    if (i != n - 1) {
      ...
      chkmin(srt, abs(q - p));
      if (!pq_le.empty()) {
        if (int r = pq_le.top(); r > q) chkmin(srt, abs(r - p));
      }
      if (!pq_gt.empty()) {
        if (int r = pq_gt.top(); r > q) chkmin(srt, abs(r - p));
      }
    } else {
      if (!pq_le.empty()) chkmin(srt, abs(pq_le.top() - p));
      if (!pq_gt.empty()) chkmin(srt, abs(pq_gt.top() - p));
    }
    chkmin(ans, srt);
    pq_gt.push(a[i].second);
  }
  cout << ans << '\n';
}
int main() {
