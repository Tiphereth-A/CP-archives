...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  cin >> n;
  vvc<int> mp(10);
  auto hf = [](int c0, int c1, int c2) -> int {
    if (c0 == 3 && c1 == 0 && c2 == 0) return 0;
    if (c0 == 2 && c1 == 1 && c2 == 0) return 1;
    if (c0 == 1 && c1 == 2 && c2 == 0) return 2;
    if (c0 == 0 && c1 == 3 && c2 == 0) return 3;
    if (c0 == 0 && c1 == 2 && c2 == 1) return 4;
    if (c0 == 0 && c1 == 1 && c2 == 2) return 5;
    if (c0 == 0 && c1 == 0 && c2 == 3) return 6;
    if (c0 == 1 && c1 == 0 && c2 == 2) return 7;
    if (c0 == 2 && c1 == 0 && c2 == 1) return 8;
    if (c0 == 1 && c1 == 1 && c2 == 1) return 9;
  };
  for_(i, 1, n) {
    string s;
    cin >> s;
    ...
    mp[hf(c0, c1, c2)].push_back(i);
  }
  vc<tuple<int, char, int, char>> ans;
  auto exchange = [&ans](auto &&f,
                         char ch_a,
                         char ch_b,
                         vc<int> &a,
                         vc<int> &b,
    ...
    if (a.size() < b.size()) return f(f, ch_b, ch_a, b, a, res_b, res_a);
    while (!b.empty()) {
      ans.emplace_back(a.back(), ch_a, b.back(), ch_b);
      res_a.push_back(a.back());
      res_b.push_back(b.back());
      a.pop_back();
      b.pop_back();
    }
  };
  auto exchange3 =
    [&ans](char ch_a, char ch_b, char ch_c, vc<int> &a, vc<int> &b, vc<int> &c)
    -> void {
    if (a.empty()) return;
    assert(a.size() == b.size() && b.size() == c.size());
    while (!a.empty()) {
      ans.emplace_back(a.back(), ch_a, b.back(), ch_b);
      ans.emplace_back(a.back(), ch_b, c.back(), ch_c);
      a.pop_back();
      b.pop_back();
      c.pop_back();
    }
  ...
  exchange(exchange,
           ...
           mp[hf(1, 1, 1)],
           mp[hf(1, 1, 1)]);
  exchange3('n', 'w', 'i', mp[hf(1, 0, 2)], mp[hf(2, 1, 0)], mp[hf(0, 2, 1)]);
  exchange3('n', 'i', 'w', mp[hf(0, 1, 2)], mp[hf(1, 2, 0)], mp[hf(2, 0, 1)]);
  for_(i, 0, 9) debug(i, mp[i]);
  cout << ans.size() << '\n';
