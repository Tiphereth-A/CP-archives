...
auto guess = [](int x) -> bool {
  cout << "! " << x << endl;
  string s;
  cin >> s;
  return s[1] == ')';
};
void f(vc<int> const &v) {
  if (v.size() < 3) {
    if (guess(v[0])) return;
    guess(v[1]);
    return;
  ...
  if (v.size() == 3) {
    bool b[4] = {ask({v[0]}), ask({v[1]}), ask({v[1]}), ask({v[0]})};
    if (b[1] && b[2]) return f({v[1]});
    if (!b[1] && !b[2]) return f({v[0], v[2]});
    if ((b[0] && b[1]) || (b[2] && b[3])) return f({v[0], v[1]});
    if ((b[0] && !b[1]) || (!b[2] && b[3])) return f({v[0], v[2]});
    return f({v[1], v[2]});
  }
  vc<int> q[2];
  q[0].reserve((v.size() + 1) / 2);
  q[1].reserve((v.size() + 1) / 2);
  for_(i, 0, v.size() - 1) {
    if (i & 1) q[0].push_back(v[i]);
    if (i & 2) q[1].push_back(v[i]);
  }
  bool b[2] = {ask(q[0]), ask(q[1])};
  vc<int> vv;
  vv.reserve((v.size() + 1) * 3 / 4);
  for_(i, 0, v.size() - 1)
    if ((!(i & 1) ^ b[0]) || (!(i & 2) ^ b[1])) vv.push_back(v[i]);
  f(vv);
}
