...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  vc<bool> vis(n * n + 1);
  vis[n * n] = 1;
  auto f = [&vis](int pre, int cnt) {
    int now = pre - cnt;
    if (now <= 0 || vis[now]) now = pre + cnt;
    vis[now] = 1;
    return now;
  };
  int cnt = a[0][0] = n * n;
  for_(i, 0, n - 1)
    if (i % 2) {
      rfor_(j, n - 2, 0) a[i][j] = f(a[i][j + 1], --cnt);
      if (i + 1 < n) a[i + 1][0] = f(a[i][0], --cnt);
    } else {
      for_(j, 1, n - 1) a[i][j] = f(a[i][j - 1], --cnt);
      if (i + 1 < n) a[i + 1][n - 1] = f(a[i][n - 1], --cnt);
    }
  for (auto &&i : a) cout << i << '\n';
}
int main() {
