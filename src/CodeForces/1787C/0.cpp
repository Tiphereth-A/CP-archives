...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  i64 n, s;
  cin >> n >> s;
  vc<pii64> f(n + 1), aa(n + 1);
  for_(i, 1, n, x) {
    cin >> x;
    if (i == 1 || i == n) aa[i] = {x, x};
    else if (x <= s) aa[i] = {0, x};
    else aa[i] = {s, x - s};
  }
  for_(i, 1, n) {
    f[i].first = min(f[i - 1].first + aa[i - 1].second * aa[i].first,
                     f[i - 1].second + aa[i - 1].first * aa[i].first);
    f[i].second = min(f[i - 1].first + aa[i - 1].second * aa[i].second,
                      f[i - 1].second + aa[i - 1].first * aa[i].second);
  }
  cout << f[n].first << '\n';
}
int main() {
