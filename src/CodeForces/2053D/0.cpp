...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  cin >> a >> b;
  auto c = a, d = b;
  ranges::sort(c), ranges::sort(d);
  mint ans = inner_product(
    c.begin(), c.end(), d.begin(), mint{1}, multiplies<>{}, [](int x, int y) {
      return min(x, y);
    });
  cout << ans << ' ';
  auto f = [&ans, n](veci &a, veci &c, veci &d, int x) {
    auto p = ranges::lower_bound(c, a[x]++) - c.begin();
    ans /= min(c[p]++, d[p]);
    if (p != n - 1 && c[p] > c[p + 1]) {
      auto q = prev(lower_bound(c.begin() + p + 1, c.end(), c[p])) - c.begin();
      ans /= min(c[q], d[q]);
      ans *= 1ll * min(c[q], d[p]) * min(c[p], d[q]);
      swap(c[p], c[q]);
    } else ans *= min(c[p], d[p]);
  };
  for (int i = 1; i <= q; i++) {
    int o, x;
    cin >> o >> x;
