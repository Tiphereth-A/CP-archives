...
const i64 MOD = 1e9 + 7;
using namespace std;
constexpr i64 inverse(i64 n, i64 mod) {
  i64 b = mod, m0 = 0;
  for (i64 q = 0, _ = 0, m1 = 1; n;) {
    _ = b - n * (q = b / n);
    b = n;
    n = _;
    _ = m0 - m1 * q;
...
void init_fact(int64_t n, int64_t mod = MOD) {
...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  vc<i64> sz(n + 1);
  i64 ans = 0;
  auto f = [&](auto &&f, int now, int fa) -> void {
    sz[now] = 1;
    for (auto &&to : tr[now]) {
      if (to == fa) continue;
      f(f, to, now);
      sz[now] += sz[to];
      ans = (ans + mCn(sz[to], k / 2) * mCn(n - sz[to], k / 2) % MOD) % MOD;
    }
  };
  f(f, 1, 0);
  ans = (ans * inverse(mCn(n, k), MOD) % MOD + 1) % MOD;
  cout << ans << '\n';
}
int main() {
