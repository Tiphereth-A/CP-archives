...
using namespace std;
using i64 = int64_t;
#define _for(i, pre, suf, vals...) for (decltype(pre + suf) i = (pre), i##end = (suf), ##vals; i <= i##end; ++i)
#define _rfor(i, suf, pre, vals...) for (make_signed_t<decltype(suf - pre)> i = (suf), i##end = (pre), ##vals; i >= i##end; --i)
const int N = 3e3 + 5;
int tr[N * N];
int cnt_a;
...
int main() {
    ...
    _for(k, 1, m)
        for (int i = 1, j = k; j >= 1 && i <= n; ++i, --j) a[id[i][j] = ++cnt_a] = {cnt_a, i - suf[i][j] + 1};
    _for(k, 2, n)
        for (int i = k, j = m; j >= 1 && i <= n; ++i, --j) a[id[i][j] = ++cnt_a] = {cnt_a, i - suf[i][j] + 1};
    sort(a + 1, a + 1 + cnt_a);
    int cnt_b = 0;
    _for(i, 1, n, len)
        _for(j, 1, m) {
