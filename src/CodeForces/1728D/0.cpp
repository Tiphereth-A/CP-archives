...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  cin >> s;
  auto &&len = s.size();
  vvc<int> dp(len + 1, vc<int>(len + 1));
  for_(r, 0, len)
    rfor_(l, r - 2, 0) {
      auto &&s1 = dp[l + 2][r] ? dp[l + 2][r] : s[l] - s[l + 1];
      auto &&s2 = dp[l + 1][r - 1] ? dp[l + 1][r - 1] : s[l] - s[r - 1];
      auto &&s3 = dp[l + 1][r - 1] ? dp[l + 1][r - 1] : s[r - 1] - s[l];
      auto &&s4 = dp[l][r - 2] ? dp[l][r - 2] : s[r - 1] - s[r - 2];
      dp[l][r] = min(max(s1, s2), max(s3, s4));
    }
  auto &&ans = dp[0][len];
  cout << (ans < 0 ? "Alice" : ans == 0 ? "Draw" : "Bob") << '\n';
}
int main() {
