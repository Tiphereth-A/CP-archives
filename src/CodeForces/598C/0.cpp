...
struct Point2 {
    ...
    constexpr Point2(data_t x = data_t{}, data_t y = data_t{}):
        x(x), y(y) {}
    friend istream &operator>>(istream &is, self &rhs) { return is >> rhs.x >> rhs.y; }
    friend ostream &operator<<(ostream &os, const self &rhs) { return os << rhs.x << ' ' << rhs.y; }
    constexpr self &set_as_polar(data_t r, data_t theta) { return *this = {r * cos(theta), r * sin(theta)}; }
    friend constexpr self lerp(const self &lhs, const self &rhs, data_t coord) { return lhs * coord + rhs * (1 - coord); }
    friend constexpr self mid_point(const self &lhs, const self &rhs) { return lerp(lhs, rhs, 0.5); }
#define SCALA_OP(op)                           \
...
#define VEC_OP(op)                                    \
    ...
    constexpr self operator op(const self &rhs) const { return self(*this) op## = rhs; }
    VEC_OP(+)
    VEC_OP(-)
#undef VEC_OP
    ...
    constexpr bool operator==(const self &rhs) const { return is_equal(x, rhs.x) && is_equal(y, rhs.y); }
    friend constexpr data_t dot_mul(const self &lhs, const self &rhs) { return lhs.x * rhs.x + lhs.y * rhs.y; }
    friend constexpr data_t cross_mul(const self &lhs, const self &rhs) { return lhs.x * rhs.y - lhs.y * rhs.x; }
#define SCALA_FUNC(name, func_body)        \
    ...
    SCALA_FUNC(arg, { data_t tmp_= atan2(y, x); return is_negative(tmp_) ? tmp_ + 2 * PI : tmp_; })
...
#undef SELF_FUNC
    ...
    friend constexpr self rotate(self lhs, data_t theta) { return lhs.do_rotate(theta); }
};
...
struct Triangle {
    ...
    point_t A, B, C;
    constexpr Triangle(const point_t &a = point_t{}, const point_t &b = point_t{}, const point_t &c = point_t{}):
        A(a), B(b), C(c) {}
    ...
    friend istream &operator>>(istream &is, self &rhs) { return is >> rhs.A >> rhs.B >> rhs.C; }
    friend ostream &operator<<(ostream &os, const self &rhs) { return os << rhs.A << ' ' << rhs.B << ' ' << rhs.C; }
    constexpr data_t area() const { return abs(cross(A, B, C)) / 2; }
    constexpr point_t average_w(data_t wA, data_t wB, data_t wC) const { return (A * wA + B * wB + C * wC) / (wA + wB + wC); }
...
struct Line2 {
    ...
    using point_t = Point2;
    point_t l, r;
    constexpr Line2(const point_t &start_ = point_t{}, const point_t &end_ = point_t{}):
        l(start_), r(end_) {}
    ...
    friend istream &operator>>(istream &is, self &rhs) { return is >> rhs.l >> rhs.r; }
    friend ostream &operator<<(ostream &os, const self &rhs) { return os << rhs.l << ' ' << rhs.r; }
    ...
    friend constexpr bool is_parallel(const self &lhs, const self &rhs) { return is_zero(cross_mul(lhs.direction(), rhs.direction())); }
    friend constexpr bool is_same_dir(const self &lhs, const self &rhs) { return is_parallel(lhs, rhs) && is_positive(dot_mul(lhs.direction(), rhs.direction())); }
    constexpr point_t direction() const { return r - l; }
    constexpr bool is_include(const point_t &p) const { return is_positive(cross(l, r, p)); }
    ...
    friend constexpr self push(self line, data_t eps = 1e-8) { return line.do_push(eps); }
};
...
constexpr bool is_on_S(const Line2 &s1, const Point2 &p) { return is_zero(cross(s1.l, s1.r, p)) && is_in_middle(s1.l, p, s1.r); }
...
constexpr data_t dist_SS(const Line2 &s1, const Line2 &s2) {
    ...
    return min({dist_PPS(s1, s2.l), dist_PPS(s1, s2.r), dist_PPS(s2, s1.l), dist_PPS(s2, s1.r)});
}
struct Circle2 {
    using self = Circle2;
    using point_t = Point2;
    point_t o;
    data_t r;
    constexpr Circle2(const point_t &c, data_t r):
        o(c), r(r) {}
    constexpr Circle2(const data_t &c_x, const data_t &c_y, data_t r_):
        o(c_x, c_y), r(r_) {}
    friend istream &operator>>(istream &is, self &rhs) { return is >> rhs.o >> rhs.r; }
    friend ostream &operator<<(ostream &os, const self &rhs) { return os << rhs.o << ' ' << rhs.r; }
...
inline cont_t<Point2> ins_CL(const Circle2 &c, const Line2 &l1) {
    if (is_greater(abs(cross_mul(c.o - l1.l, l1.r - l1.l) / dist_PP(l1.l, l1.r)), c.r)) return {};
    data_t x = dot_mul(l1.l - c.o, l1.r - l1.l), y = (l1.r - l1.l).norm(), d = max(x * x - y * ((l1.l - c.o).norm() - c.r * c.r), data_t{0});
...
struct Polynomial2 {
    ...
    Polynomial2(const cont_t<point_t> &_vertice):
        vertice(_vertice) {}
    ...
    friend ostream &operator<<(ostream &os, const self &rhs) {
        for (auto it = rhs.vertice.begin(); it != rhs.vertice.end() - 1; ++it) os << *it << ' ';
    ...
    inline point_t &operator[](size_t x) { return vertice[x]; }
    inline const point_t &operator[](size_t x) const { return vertice[x]; }
#define SCALA_FUNC(name, func_body)     \
    inline auto name() const func_body; \
    friend inline auto name(const self &lhs) { return lhs.name(); }
    SCALA_FUNC(circum, {
        data_t result{};
        size_t sz = vertice.size();
        for (size_t i = 0; i < sz; ++i) result += dist_PP(vertice[i], vertice[(i + 1) % sz]);
        return result;
    })
    SCALA_FUNC(area, {
        data_t result{};
        size_t sz = vertice.size();
        for (size_t i = 0; i < sz; ++i) result += cross_mul(vertice[i], vertice[(i + 1) % sz]);
        return result / 2;
    })
    SCALA_FUNC(convex_diameter, {
        ...
        do {
            (++(cross_mul(vertice[(i + 1) % sz] - vertice[i], vertice[(j + 1) % sz] - vertice[j]) >= 0 ? j : i)) %= sz;
...
#undef SCALA_FUNC
    ...
    static const size_t on_boarder = 1;
    static const size_t outside = 0;
    inline size_t is_containing(const point_t &p) const {
        bool result = 0;
        size_t sz = vertice.size();
    ...
    inline size_t is_containing_cvh(const point_t &p) const {
        if (is_positive(cross_mul(vertice.back(), p))) return self::outside;
        auto it = std::lower_bound(vertice.begin(), vertice.end(), p, [](const point_t &lhs, const point_t &rhs) { return is_positive(cross_mul(lhs, rhs)) || (is_zero(cross_mul(lhs, rhs)) && lhs.norm() < rhs.norm()); });
        return is_equal(std::abs(cross_mul(*(it - 1), p)) + std::abs(cross_mul(p, *it)) + std::abs(cross(p, *(it - 1), *it)), std::abs(cross_mul(*(it - 1), *it))) ? self::inside : self::outside;
    }
#define SELF_FUNC(name, func_body)      \
    ...
    SELF_FUNC(convex_hull, {
        ...
        for (ptrdiff_t i = 0; i < sz; cvh[sz_cvh++] = vertice[i++])
            while (sz_cvh > 1 && cross_sign(cvh[sz_cvh - 2], cvh[sz_cvh - 1], vertice[i]) <= 0) --sz_cvh;
        for (ptrdiff_t i = sz - 2, t = sz_cvh; ~i; cvh[sz_cvh++] = vertice[i--])
            while (sz_cvh > t && cross_sign(cvh[sz_cvh - 2], cvh[sz_cvh - 1], vertice[i]) <= 0) --sz_cvh;
        cvh.resize(sz_cvh - 1);
        vertice = cvh;
        return *this;
    })
...
#undef SELF_FUNC
    inline self &do_convex_cut(const point_t &q1, const point_t &q2) {
        size_t sz = vertice.size();
        cont_t<point_t> cvc;
        for (size_t i = 0; i < sz; ++i) {
    ...
    friend inline self convex_cut(self poly, const point_t &q1, const point_t &q2) { return poly.do_convex_cut(q1, q2); }
};
...
inline Polynomial2 ins_hP(cont_t<Line2> vl) {
