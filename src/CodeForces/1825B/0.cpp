...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  for (auto &i : b) cin >> i;
  sort(b.begin(), b.end());
  i64 max1 = b[m * n - 1], max2 = b[m * n - 2], min1 = b[0], min2 = b[1];
  i64 ans1 = (max1 - min1) * ((n - 1) * m) + (max1 - min2) * (m - 1);
  i64 ans2 = (max1 - min1) * (n * (m - 1)) + (max1 - min2) * (n - 1);
  i64 ans3 = (max1 - min1) * ((n - 1) * m) + (max2 - min1) * (m - 1);
  i64 ans4 = (max1 - min1) * (n * (m - 1)) + (max2 - min1) * (n - 1);
