...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  cin >> x >> m;
  i64 ans = 0;
  for_(y, 1, min(x, m)) ans += !((x ^ y) % y);
  dbg(ans);
  if (m > x) {
    i64 amax = m / x - 1, amin = 2;
    for_(a, amax + 1, amax + 10) {
      i64 y = (a * x) ^ x;
      ans += (y >= 1) && (y <= m);
    }
    while (amax > 0 && ((amax * x) ^ x) > m) --amax;
    while (amin <= amax && ((amin * x) ^ x) <= x) ++amin;
    dbg(amax, amin);
    ans += max<i64>(0, amax - amin + 1);
  }
  cout << ans << '\n';
}
int main() {
