...
inline auto solve() -> void {
    ...
    if (f1) {
        ...
        _for(i, 1, odds.size() - 2) {
            ...
            for (int j = _next(_next(odds[i - 1])); j != _next(odds[i]); j = _next(j)) print(_prev(j), j);
        }
        print(r, _next(odds[odds.size() - 2]));
        for (int j = _next(_next(odds[odds.size() - 2])); j != odds.back(); j = _next(j)) print(_prev(j), j);
        print(_prev(odds.back()), _prev(r));
        for (int j = _prev(_prev(r)); j != _prev(odds.back()); j = _prev(j)) print(_next(j), j);
        return;
    }
    bool f = 0;
    int a = r;
