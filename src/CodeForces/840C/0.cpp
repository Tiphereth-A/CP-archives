...
using mint = MODINT::Mint<1000000007>;
using namespace std;
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  auto f = [](int x) -> int {
    int ans = 1;
    for (int i = 2, sqx = sqrt(x); i <= sqx; ++i)
      if (x % i == 0) {
        bool f = false;
        while (x % i == 0) {
          f = !f;
  ...
  for_(i, 1, n) a[i] = f(a[i]);
  sort(range_(a, 1, n + 1));
  ndvc<2, mint> now(n + 2, n + 2), nxt(n + 2, n + 2);
  now[0][0] = 1;
  for_(i, 1, n, cnt = 0) {
    if (a[i] != a[i - 1]) {
      for_(j, 0, i) {
        nxt[j][0] = accumulate(range_(nxt[j], 0, cnt + 1), mint(0));
        for_(k, 1, cnt) nxt[j][k] = 0;
      }
      cnt = 0;
    }
    for_(j, 0, i) {
      for_(k, 0, min(j, cnt)) {
        if (j && k) now[j][k] += nxt[j - 1][k - 1] * (cnt * 2 - k + 1);
        now[j][k] +=
          nxt[j + 1][k] * (j + 1 - k) + nxt[j][k] * (i - cnt * 2 + k * 2 - j);
      }
    }
    now.swap(nxt);
    now.fill(0);
