...
#define for_(i, l, r, ...)                                            \
  for (std::make_signed_t<decltype(l + r)> i = (l),                   \
       ...
       ++i)
#define rfor_(i, r, l, ...)                                           \
  for (std::make_signed_t<decltype(r - l)> i = (r),                   \
                                           i##end = (l)__VA_OPT__(, ) \
                                             __VA_ARGS__;             \
       i >= i##end;                                                   \
       --i)
  ...
  for_(i, 0, (1 << n) - 1) {
    ...
    if (chkmax(mx, sum)) {
      ...
      for (auto j : v) {
        if (j > l) {
          set<i64> s;
          for_(k, l, j - 1) s.insert(a[k]);
          int mex = 0;
          while (s.contains(mex)) mex++;
          if (mex) ans.emplace_back(l, j - 1);
          ans.emplace_back(l, j - 1);
          auto f = [&](auto &&f, int l, int x) -> void {
            if (x == 1) {
              ans.emplace_back(l, l);
              return;
            }
            rfor_(k, x - 1, 1) f(f, l + (x - 1 - k), k);
            ans.emplace_back(l, j - 1);
            ans.emplace_back(l + 1, j - 1);
          };
          int len = j - l;
          rfor_(k, len - 1, 1) f(f, l + (len - 1 - k), k);
          ans.emplace_back(l, j - 1);
        }
        l = j + 1;
      }
