...
template <class _T = std::ptrdiff_t, const std::size_t __N = N, const bool _clear = false>
class SegTree {
  private:
    struct Node {
        _T sum, max, lazy;
    ...
    std::size_t n;
    inline void _push_up(std::size_t index) {
        t[index].sum = t[index << 1].sum + t[index << 1 | 1].sum;
        t[index].max = max(t[index << 1].max, t[index << 1 | 1].max);
    }
    inline void _push_down(std::size_t index, std::ptrdiff_t len_l, std::ptrdiff_t len_r) {
    ...
    void _init(_T data[], std::size_t index = 1, std::size_t l = 1, std::size_t r = __N - 1) {
        if (l == r) {
    ...
    void _modify(std::size_t pos, _T k, std::size_t index = 1, std::size_t l = 1, std::size_t r = __N - 1) {
        if (l == r) {
        ...
        if (mid >= pos)
            _modify(pos, k, index << 1, l, mid);
        else
            _modify(pos, k, index << 1 | 1, mid + 1, r);
        _push_up(index);
    }
    void _update(std::size_t L, std::size_t R, _T k, std::size_t index = 1, std::size_t l = 1, std::size_t r = __N - 1) {
        if (L <= l && r <= R) {
            if (t[index].max < k)
                return;
        }
    ...
    _T _query(std::size_t L, std::size_t R, std::size_t index = 1, std::size_t l = 1, std::size_t r = __N - 1) {
  ...
  public:
    ...
    void update(std::size_t Left, std::size_t Right, _T Val) { _update(Left, Right, Val, 1, 1, n); }
    void modify(std::size_t pos, _T val) { _modify(pos, val, 1, 1, n); }
    _T query(std::size_t Left, std::size_t Right) { return _query(Left, Right, 1, 1, n); }
};
i64 a[N];
SegTree<i64> st;
int main() {
