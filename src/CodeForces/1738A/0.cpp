...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  vc<i64> v0, v1;
  for_(i, 0, n - 1) (a[i] ? v1 : v0).push_back(b[i]);
  sort(all_(v0), greater<i64>());
  sort(all_(v1), greater<i64>());
  if (v0.size() > v1.size()) swap(v0, v1);
  i64 ans = accumulate(all_(v0), i64(0)) +
            accumulate(v1.begin(), v1.begin() + v0.size(), i64(0));
  ans *= 2;
  if (v1.size() == v0.size()) ans -= min(v1.back(), v0.back());
  else ans += accumulate(v1.begin() + v0.size(), v1.end(), i64(0));
  cout << ans << '\n';
}
int main() {
