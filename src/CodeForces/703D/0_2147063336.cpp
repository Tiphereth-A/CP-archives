...
class BIT {
  private:
    _T tree[_N];
    inline constexpr std::size_t lowbit(std::ptrdiff_t x) { return x & (-x); }

  public:
    BIT() {}
    inline void add(std::size_t pos, _T val = 1) {
        for (std::size_t i = pos; i < _N; i += lowbit(i)) tree[i] ^= val;
    }
    inline _T query(std::size_t pos) {
        _T ret = 0;
...
int main() {
    ...
    _foreach_cref(i, q) {
        ...
        ans[get<2>(i)] = tr.query(get<1>(i)) ^ tr.query(get<0>(i) - 1) ^ sum[get<1>(i)] ^ sum[get<0>(i) - 1];
    }
