...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  if (y) {
    ...
    ivl.reserve(x);
    for_(i, 0, x - 1)
      if (u32 _ = a[i + 1] - a[i] - 1; _ > 1) ivl.emplace_back(_, a[i]);
    sort(all_(ivl), [](auto const &l, auto const &r) -> bool {
      if ((l.first ^ r.first) & 1) return l.first & 1;
      return l < r;
    });
    for (auto [l, idx] : ivl) {
      u32 max_node = min(y, l / 2);
      ans += max_node * 2;
      if (max_node * 2 + 1 == l) ++ans;
      y -= max_node;
      if (!y) break;
    }
  }
