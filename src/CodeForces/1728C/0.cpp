...
using namespace std;
constexpr int digit10(int x) { return x < 1 ? 1 : (int)ceil(log10(x + 1)); }
auto solve([[maybe_unused]] int t_ = 0) -> void {
  read_var_(int, n);
  read_container_(vc<int>, a, n);
  read_container_(vc<int>, b, n);
  sort(all_(a));
  sort(all_(b));
  vector<int> same, new_v;
  set_intersection(all_(a), all_(b), ins_(same));
  if (same.size() == a.size()) run_return_void_(cout << "0\n");
  if (same.size()) {
    set_difference(all_(a), all_(same), ins_(new_v));
    a = new_v;
    new_v.clear();
    set_difference(all_(b), all_(same), ins_(new_v));
    b = new_v;
    new_v.clear();
  }
  int ans = 0;
  ...
  same.clear();
  set_intersection(all_(a), all_(b), ins_(same));
  if (same.size() == a.size()) run_return_void_(cout << ans << '\n');
  if (same.size()) {
    set_difference(all_(a), all_(same), ins_(new_v));
    a = new_v;
    new_v.clear();
  ...
  cout << ans + a.size() * 2 - (count(all_(a), 1) + count(all_(b), 1)) << '\n';
}
int main() {
