    ...
    while (kase--) {
        ...
        _for(i, 1, n) {
            ...
            _for(k, i + 1, n) {
                ...
                _for(j, 1, n) {
                    if (j == i || j == k) continue;
                    if (maps[i][j] == maps[j][k]) _run_break(b = _b = 1; x = i; y = j; z = k);
                }
                if (_b == 1) break;
            }
            if (b == 1) break;
        ...
        cout << "YES\n";
        if (m % 4 == 0) _run_continue(_for(i, 1, (m - 1) / 2) cout << (i % 2 ? y : x) << ' '; cout << x << ' ' << y << ' ' << z << ' '; _for(i, 1, (m - 1) / 2) cout << (i % 2 ? y : z) << " \n"[i == (m - 1) / 2];);
        _for(i, 1, m + 1) cout << (i % 4 == 1 ? x : (i % 2 == 0 ? y : z)) << " \n"[i == m + 1];
    }
}
