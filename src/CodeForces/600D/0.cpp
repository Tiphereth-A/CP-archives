...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  cin >> p1 >> r1 >> p2 >> r2;
  pii64 _ = p2 - p1;
  i64 dist = _.first * _.first + _.second * _.second;
  cout << fixed << setprecision(20);
  if (dist >= (r1 + r2) * (r1 + r2)) run_return_void_(cout << 0.l << '\n');
  if (dist <= (r1 - r2) * (r1 - r2))
    run_return_void_(cout << PI * min(r1 * r1, r2 * r2) << '\n');
  i64 r12 = r1 * r1, r22 = r2 * r2;
  long double theta1 = 2 * acosl((dist - r22 + r12) / (2 * sqrtl(dist) * r1)),
              theta2 = 2 * acosl((dist - r12 + r22) / (2 * sqrtl(dist) * r2));
  cout << (r1 * r1 * (theta1 - sin(theta1)) +
           r2 * r2 * (theta2 - sin(theta2))) /
            2
       << '\n';
}
int main() {
