...
struct Polynomial2 {
    ...
    inline std::vector<Point2>::const_iterator prev(std::vector<Point2>::const_iterator it) const { return --(it == vs.begin() ? it = vs.end() : it); }
    inline std::vector<Point2>::const_iterator next(std::vector<Point2>::const_iterator it) const { return ++it == vs.end() ? vs.begin() : it; }
...
inline void solve(int t_) {
    ...
    while ((cin >> n), ~n) {
        ...
        for (size_t i = 0; i < cvh.vs.size(); ++i) {
            ...
            while (f) {
                f = false;
                if (is_greater(new_tmp = cross(cvh[i], cvh[j], cvh[cvh.next(k)]), tmp)) {
                    k = cvh.next(k);
                    ans = std::max(ans, tmp = new_tmp);
                    f = true;
                }
                if (is_greater(new_tmp = cross(cvh[i], cvh[cvh.next(j)], cvh[k]), tmp)) {
                    j = cvh.next(j);
                    ans = std::max(ans, tmp = new_tmp);
                    f = true;
                }
