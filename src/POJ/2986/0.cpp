...
#define SCALA_FUNC(name, func_body) \
  ...
  int quad() const { return QUAD[(sign(y) + 1) * 3 + sign(x) + 1]; }
...
struct Line2 {
  ...
  friend Line2 push(Line2 line, data_t eps = 1e-8) { return line.do_push(eps); }
};
...
std::vector<Circle2> make_C_rPP(data_t r, const Point2 &p1, const Point2 &p2) {
...
std::vector<Circle2> make_C_rPL(data_t r, const Point2 &p, const Line2 &l) {
...
std::vector<Circle2> make_C_rLL(data_t r, const Line2 &l1, const Line2 &l2) {
...
std::vector<Circle2>
make_C_rCC_ex(data_t r, const Circle2 &c1, const Circle2 &c2) {
  if (c1.relation_C(c2) == Circle2::inside) return std::vector<Circle2>();
  std::vector<Point2> ps =
    ins_CC(Circle2(c1.o, c1.r + r), Circle2(c2.o, c2.r + r));
  std::vector<Circle2> ret;
  for (size_t i = 0; i < ps.size(); ++i) ret.push_back(Circle2(ps[i], r));
  return ret;
}
...
data_t area_CC(const Circle2 &lhs, const Circle2 &rhs) {
...
bool min_dist_Ps_cmpy(const Point2 &lhs, const Point2 &rhs) {
  return lhs.y < rhs.y;
}
...
struct Polynomial2 {
  ...
  std::vector<Point2>::iterator prev(std::vector<Point2>::iterator it) {
  ...
  std::vector<Point2>::iterator next(std::vector<Point2>::iterator it) {
...
int main() {
  ...
  Point2 p1, p2, p3;
  Circle2 c;
  while (cin >> p1 >> p2 >> p3 >> c)
    cout << abs(sarea_CT(c, p1, p2) + sarea_CT(c, p2, p3) + sarea_CT(c, p3, p1))
         << '\n';
  return 0;
}
