...
std::basic_ostream<Ch, Tr> &operator<<(std::basic_ostream<Ch, Tr> &os, const Container &x) {
    if (&os == &std::cerr) os << "[";
    if (&os == &std::cerr)
        for (typename Container::const_iterator it = x.begin(); it != x.end() - 1; ++it) os << *it << ", ";
    else
        for (typename Container::const_iterator it = x.begin(); it != x.end() - 1; ++it) os << *it << ' ';
    os << x.back();
    if (&os == &std::cerr) os << "]";
...
const int QUAD[9] = {5, 6, 7, 4, 0, 0, 3, 2, 1};
...
inline uint_data_t f2uint(data_t x) { return uint_data_t(x + EPS); }
...
inline ptrdiff_t comp_rerr(data_t lhs, data_t rhs) { return sign((lhs - rhs) / std::abs(lhs)); }
inline bool is_less_rerr(data_t lhs, data_t rhs) { return is_negative((lhs - rhs) / std::abs(lhs)); }
inline bool is_equal_rerr(data_t lhs, data_t rhs) { return is_zero((lhs - rhs) / std::abs(lhs)); }
inline bool is_greater_rerr(data_t lhs, data_t rhs) { return is_positive((lhs - rhs) / std::abs(lhs)); }
...
struct Point2 {
    data_t x, y;
    Point2(data_t x = data_t(0), data_t y = data_t(0)):
        x(x), y(y) {}
    ...
    inline Point2 &set_as_polar(data_t r, data_t theta) { return *this = Point2(r * cos(theta), r * sin(theta)); }
    friend inline Point2 lerp(const Point2 &lhs, const Point2 &rhs, data_t coord) { return lhs * coord + rhs * (1 - coord); }
    friend inline Point2 mid_point(const Point2 &lhs, const Point2 &rhs) { return lerp(lhs, rhs, 0.5); }
#define SCALA_OP(op)                          \
    inline Point2 &operator op##=(data_t n) { \
        this->x op## = n;                     \
        this->y op## = n;                     \
        return *this;                         \
    }                                         \
    inline Point2 operator op(data_t n) const { return Point2(*this) op## = n; }
    SCALA_OP(+)
    SCALA_OP(-)
    SCALA_OP(*)
    SCALA_OP(/)
#undef SCALA_OP
#define VEC_OP(op)                                     \
    inline Point2 &operator op##=(const Point2 &rhs) { \
        this->x op## = rhs.x;                          \
        this->y op## = rhs.y;                          \
        return *this;                                  \
    }                                                  \
    inline Point2 operator op(const Point2 &rhs) const { return Point2(*this) op## = rhs; }
    VEC_OP(+)
    VEC_OP(-)
#undef VEC_OP
    ...
    inline bool operator==(const Point2 &rhs) const { return is_equal(x, rhs.x) && is_equal(y, rhs.y); }
    inline bool operator!=(const Point2 &rhs) const { return !(*this == rhs); }
    friend inline data_t dot_mul(const Point2 &lhs, const Point2 &rhs) { return lhs.x * rhs.x + lhs.y * rhs.y; }
    friend inline data_t cross_mul(const Point2 &lhs, const Point2 &rhs) { return lhs.x * rhs.y - lhs.y * rhs.x; }
#define SCALA_FUNC(name, func_body)       \
    inline data_t name() const func_body; \
    friend inline data_t name(const Point2 &lhs) { return lhs.name(); }
    ...
    inline int quad() const { return QUAD[(sign(y) + 1) * 3 + sign(x) + 1]; }
    friend inline int quad(const Point2 &lhs) { return lhs.quad(); }
#undef SCALA_FUNC
#define SELF_FUNC(name, func_body)        \
    inline Point2 &do_##name() func_body; \
    friend inline Point2 name(Point2 lhs) { return lhs.do_##name(); }
...
#undef SELF_FUNC
    inline Point2 &do_rot(data_t theta) {
        data_t _x = x, _y = y;
        x = _x * cos(theta) - _y * sin(theta);
        y = _x * sin(theta) + _y * cos(theta);
        return *this;
    };
    friend inline Point2 rot(Point2 lhs, data_t theta) { return lhs.do_rot(theta); }
};
inline data_t dist_PP(const Point2 &lhs, const Point2 &rhs) { return std::sqrt((lhs.x - rhs.x) * (lhs.x - rhs.x) + (lhs.y - rhs.y) * (lhs.y - rhs.y)); }
...
struct Line2 {
    Point2 l, r;
    Line2() {}
    inline Line2(const Point2 &start_, const Point2 &end_):
        l(start_), r(end_) {}
    ...
    inline friend bool operator<(const Line2 &lhs, const Line2 &rhs) {
    ...
    friend inline bool is_parallel(const Line2 &lhs, const Line2 &rhs) { return is_zero(cross_mul(lhs.direction(), rhs.direction())); }
    friend inline bool is_same_dir(const Line2 &lhs, const Line2 &rhs) { return is_parallel(lhs, rhs) && is_positive(dot_mul(lhs.direction(), rhs.direction())); }
    inline Point2 direction() const { return r - l; }
    inline bool is_include(const Point2 &p) const { return is_positive(cross(l, r, p)); }
    inline bool passes_through(const Point2 &p) const { return is_zero(cross(l, r, p)); }
    inline Line2 &do_push(data_t eps = 1e-8) {
    ...
    friend inline Line2 push(Line2 line, data_t eps = 1e-8) { return line.do_push(eps); }
};
...
inline data_t dist_SS(const Line2 &s1, const Line2 &s2) {
    ...
    return std::min(std::min(dist_PS(s1.l, s2), dist_PS(s1.r, s2)), std::min(dist_PS(s2.l, s1), dist_PS(s2.r, s1)));
}
struct Triangle {
    Point2 A, B, C;
    Triangle() {}
    Triangle(const Point2 &a, const Point2 &b, const Point2 &c):
        A(a), B(b), C(c) {}
    Triangle(const data_t &a_x, const data_t &a_y, const data_t &b_x, const data_t &b_y, const data_t &c_x, const data_t &c_y):
    ...
    inline data_t area() const { return std::abs(cross(A, B, C)) / 2; }
    inline Point2 average_w(data_t wA, data_t wB, data_t wC) const { return (A * wA + B * wB + C * wC) / (wA + wB + wC); }
    inline bool is_acute() const { return is_positive(dot(A, B, C)) && is_positive(dot(B, C, A)) && is_positive(dot(C, A, B)); }
    inline bool is_right() const { return is_zero(dot(A, B, C)) || is_zero(dot(B, C, A)) || is_zero(dot(C, A, B)); }
    inline bool is_obtuse() const { return is_negative(dot(A, B, C)) || is_negative(dot(B, C, A)) || is_negative(dot(C, A, B)); }
    inline data_t radius_I() const {
        data_t ab = dist_PP(A, B), bc = dist_PP(B, C), ca = dist_PP(C, A);
        return 2 * area() / (ab + bc + ca);
    }
    inline data_t radius_O() const {
        data_t ab = dist_PP(A, B), bc = dist_PP(B, C), ca = dist_PP(C, A);
        return ab * bc * ca / (4 * area());
    }
    inline Point2 center_I() const { return average_w((B - C).abs(), (C - A).abs(), (A - B).abs()); }
    inline Point2 center_E() const { return average_w(-((B - C).abs()), (C - A).abs(), (A - B).abs()); }
    inline Point2 center_G() const {
        return average_w(1, 1, 1);
    }
    inline Point2 center_O() const {
        Point2 p1 = mid_point(B, C), p2 = mid_point(C, A);
        return ins_LL(Line2(p1, p1 + (B - C).do_rot90()), Line2(p2, p2 + (C - A).do_rot90()));
    }
    inline Point2 center_H() const {
        return ins_LL(Line2(A, A + (B - C).do_rot90()), Line2(B, B + (C - A).do_rot90()));
    }
    inline Point2 center_N() const {
...
inline data_t area_T_abc(data_t a, data_t b, data_t c) { return std::sqrt((a + (b + c)) * (c - (a - b)) * (c + (a - b)) * (a + (b - c))) / 4; }
struct Circle2 {
    Point2 o;
    data_t r;
    Circle2() {}
    Circle2(const Point2 &c, data_t r):
        o(c), r(r) {}
    ...
    inline size_t relation_C(const Circle2 &c2) const {
    ...
    inline size_t relation_P(const Point2 &p) const {
    ...
    inline size_t relation_L(const Line2 &l) const {
        data_t d = dist_PL(o, l);
        if (is_less(d, r)) return intersect;
        if (is_equal(d, r)) return tagante;
        return outside;
    }
    inline size_t relation_S(const Line2 &s) const {
        data_t d = dist_PS(o, s);
        if (is_less(d, r)) return intersect;
        if (is_equal(d, r)) return tagante;
        return otherwise;
    }
    inline data_t area(data_t angle = PI * 2) const { return angle * r * r / 2; }
    inline data_t crown_area(data_t angle = PI * 2) const { return (angle - std::sin(angle)) * r * r / 2; }
};
inline std::vector<Point2> ins_CL(const Circle2 &c, const Line2 &l1) {
    if (is_greater(std::abs(cross_mul(c.o - l1.l, l1.r - l1.l) / dist_PP(l1.l, l1.r)), c.r)) return std::vector<Point2>();
    data_t x = dot_mul(l1.l - c.o, l1.r - l1.l), y = l1.direction().norm(), d = std::max(x * x - y * ((l1.l - c.o).norm() - c.r * c.r), data_t(0));
...
inline std::vector<Point2> ins_CC(const Circle2 &c1, const Circle2 &c2) {
    assert(!is_equal(c1.o.x, c2.o.x) || !is_equal(c1.o.y, c2.o.y) || !is_equal(c1.r, c2.r));
    size_t state = c1.relation_C(c2);
    if (state == Circle2::lying_in || state == Circle2::lying_out) return std::vector<Point2>();
...
inline std::vector<Circle2> make_C_rPP(data_t r, const Point2 &p1, const Point2 &p2) {
    std::vector<Point2> ps = ins_CC(Circle2(p1, r), Circle2(p2, r));
    std::vector<Circle2> ret;
    for (size_t i = 0; i < ps.size(); ++i) ret.push_back(Circle2(ps[i], r));
    return ret;
}
inline std::vector<Circle2> make_C_rPL(data_t r, const Point2 &p, const Line2 &l) {
    data_t dis = dist_PL(p, l);
    if (is_positive(dis - r * 2)) return std::vector<Circle2>();
    Point2 dir = l.direction();
    dir *= r / dir.abs();
    Point2 dirl = rot90(dir), dirr = rot270(dir);
    if (is_zero(dis)) {
        std::vector<Circle2> ret;
        ret.push_back(Circle2(p + dirl, r));
        ret.push_back(Circle2(p + dirr, r));
        return ret;
    }
    Circle2 c(p, r);
    std::vector<Point2> ps = ins_CL(c, Line2(l.l + dirl, l.r + dirl));
    if (ps.size() == 0) ps = ins_CL(c, Line2(l.l + dirr, l.r + dirr));
    std::vector<Circle2> ret;
    for (size_t i = 0; i < ps.size(); ++i) ret.push_back(Circle2(ps[i], r));
    return ret;
}
inline std::vector<Circle2> make_C_rLL(data_t r, const Line2 &l1, const Line2 &l2) {
    if (is_parallel(l1, l2)) return std::vector<Circle2>();
    ...
    std::vector<Circle2> ret;
    ret.push_back(Circle2(ins_LL(u1, v1), r));
    ret.push_back(Circle2(ins_LL(u1, v2), r));
    ret.push_back(Circle2(ins_LL(u2, v1), r));
    ret.push_back(Circle2(ins_LL(u2, v2), r));
    return ret;
}
inline std::vector<Circle2> make_C_rCC_ex(data_t r, const Circle2 &c1, const Circle2 &c2) {
    if (c1.relation_C(c2) == Circle2::inside) return std::vector<Circle2>();
    std::vector<Point2> ps = ins_CC(Circle2(c1.o, c1.r + r), Circle2(c2.o, c2.r + r));
    std::vector<Circle2> ret;
    for (size_t i = 0; i < ps.size(); ++i) ret.push_back(Circle2(ps[i], r));
    return ret;
}
inline Point2 inv_P(const Circle2 &c, const Point2 &p) {
    Point2 v = p - c.o;
    return c.o + v * (c.r * c.r / v.norm());
}
inline Circle2 inv_C2C(const Circle2 &c, const Circle2 &c2) {
    Point2 v = (c2.o - c.o).do_unit();
    Point2 p1 = c2.o + v * c2.r, p2 = c2.o - v * c2.r;
    return Circle2(mid_point(p1, p2), dist_PP(p1, p2) / 2);
}
inline Line2 inv_C2L(const Circle2 &c, const Point2 &p) {
    Point2 v = (p - c.o).do_rot90();
    return Line2(inv_P(c, p + v), inv_P(c, p - v));
}
inline Circle2 inv_L(const Circle2 &c, const Line2 &l) {
    Point2 p1 = inv_P(c, l.l), p2 = inv_P(c, l.r);
    Point2 o2 = Triangle(p1, p2, c.o).center_O();
    return Circle2(o2, dist_PP(c.o, o2));
}
inline std::vector<Point2> tan_CP(const Circle2 &c, const Point2 &p) {
    Point2 v = p - c.o;
...
inline std::vector<Line2> extan_CC(const Circle2 &c1, const Circle2 &c2) {
    if (is_equal(c1.o.x, c2.o.x) && is_equal(c1.o.y, c2.o.y)) return std::vector<Line2>();
    ...
    if (is_equal(c1.r, c2.r)) {
        ...
        for (size_t i = 0; i < std::min(ps.size(), qs.size()); ++i) ret.push_back(Line2(ps[i], qs[i]));
    }
    return ret;
}
inline std::vector<Line2> intan_CC(const Circle2 &c1, const Circle2 &c2) {
    if (is_equal(c1.o.x, c2.o.x) && is_equal(c1.o.y, c2.o.y)) return std::vector<Line2>();
    std::vector<Line2> ret;
...
inline data_t sarea_CT(const Circle2 &c, const Point2 &p1, const Point2 &p2) {
    if (is_zero(cross(c.o, p1, p2))) return data_t(0);
    std::vector<Point2> is = ins_CL(c, Line2(p1, p2));
...
inline data_t area_CC(const Circle2 &lhs, const Circle2 &rhs) {
    size_t relation = lhs.relation_C(rhs);
    if (relation == Circle2::lying_out || relation == Circle2::touch_ex) return data_t(0);
...
bool min_dist_Ps_cmpy(const Point2 &lhs, const Point2 &rhs) { return lhs.y < rhs.y; }
data_t min_dist_Ps(const std::vector<Point2> &vp, ptrdiff_t l, ptrdiff_t r) {
    data_t ret = DATA_MAX;
    if (r - l <= 5) {
    ...
    std::stable_sort(q.begin(), q.end(), min_dist_Ps_cmpy);
...
struct Polynomial2 {
    ...
    explicit Polynomial2(size_t size_):
        vs(size_) {}
    explicit Polynomial2(const std::vector<Point2> &vs_):
        vs(vs_) {}
    friend std::istream &operator>>(std::istream &is, Polynomial2 &rhs) {
        for (std::vector<Point2>::iterator it = rhs.vs.begin(); it != rhs.vs.end(); ++it) is >> *it;
        return is;
    }
    friend std::ostream &operator<<(std::ostream &os, const Polynomial2 &rhs) {
        for (std::vector<Point2>::const_iterator it = rhs.vs.begin(); it != rhs.vs.end() - 1; ++it) os << *it << ' ';
    ...
    inline std::vector<Point2>::iterator prev(std::vector<Point2>::iterator it) {
        if (it == vs.begin()) it = vs.end();
        return --it;
    }
    inline std::vector<Point2>::const_iterator prev(std::vector<Point2>::const_iterator it) const {
        if (it == vs.begin()) it = vs.end();
        return --it;
    }
    inline std::vector<Point2>::iterator next(std::vector<Point2>::iterator it) {
        if (++it == vs.end()) it = vs.begin();
        return it;
    }
    inline std::vector<Point2>::const_iterator next(std::vector<Point2>::const_iterator it) const {
        if (++it == vs.end()) it = vs.begin();
        return it;
    }
...
#define SCALA_FUNC(name, func_body)       \
    inline data_t name() const func_body; \
    friend inline data_t name(const Polynomial2 &lhs) { return lhs.name(); }
    SCALA_FUNC(circum, {
        data_t result = 0;
...
#undef SCALA_FUNC
    ...
    virtual inline size_t relation_P(const Point2 &p) const {
        for (std::vector<Point2>::const_iterator it = vs.begin(); it != vs.end(); ++it)
    ...
    inline size_t relation_C(const Circle2 &c) const {
        ...
        for (size_t i = 0; i < vs.size(); ++i) {
            size_t state = c.relation_S(Line2(vs[i], vs[next(i)]));
...
int main() {
    ...
    Point2 p1, p2, p3;
    Circle2 c;
    while (cin >> p1 >> p2 >> p3 >> c) cout << abs(sarea_CT(c, p1, p2) + sarea_CT(c, p2, p3) + sarea_CT(c, p3, p1)) << '\n';
    return 0;
}
