...
class SegTree {
  private:
    ...
    void _update(std::size_t L, std::size_t R, u64 k, std::size_t index = 1, std::size_t l = 1, std::size_t r = N - 1) {
        if (L <= l && r <= R) {
            t[index] = Node(k, k);
            return;
        }
    ...
    u64 _query(std::size_t L, std::size_t R, std::size_t index = 1, std::size_t l = 1, std::size_t r = N - 1) {
