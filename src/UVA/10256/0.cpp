...
#define SCALA_FUNC(name, func_body)       \
    ...
    friend inline data_t quad(const self &lhs) { return lhs.quad(); }
#undef SCALA_FUNC
...
struct Triangle {
    ...
    point_t A, B, C;
    inline Triangle(const point_t &a = point_t{}, const point_t &b = point_t{}, const point_t &c = point_t{}):
        A(a), B(b), C(c) {}
    inline Triangle(const data_t &a_x, const data_t &a_y, const data_t &b_x, const data_t &b_y, const data_t &c_x, const data_t &c_y):
    ...
    inline data_t area() const { return abs(cross(A, B, C)) / 2; }
    inline point_t average_w(data_t wA, data_t wB, data_t wC) const { return (A * wA + B * wB + C * wC) / (wA + wB + wC); }
...
struct Line2 {
    ...
    using point_t = Point2;
    point_t l, r;
    inline Line2(const point_t &start_ = point_t{}, const point_t &end_ = point_t{}):
        l(start_), r(end_) {}
...
inline bool is_on_S(const Line2 &s1, const Point2 &p) { return is_zero(cross(s1.l, s1.r, p)) && is_in_middle(s1.l, p, s1.r); }
...
inline data_t dist_PPS(const Line2 &s1, const Point2 &p) {
...
struct Circle2 {
    ...
    point_t o;
    data_t r;
    inline Circle2(const point_t &c, data_t r):
        o(c), r(r) {}
    inline Circle2(const data_t &c_x, const data_t &c_y, data_t r_):
        o(c_x, c_y), r(r_) {}
    ...
    friend inline size_t insection_type(const Circle2 &c1, const Circle2 &c2) {
...
#undef SCALA_FUNC
    ...
    inline size_t is_containing_cvh(const point_t &p) const {
        if (vertice.size() == 1) return vertice.front() == p ? self::inside_or_onborder : self::outside;
        if (vertice.size() == 2) return is_on_S({vertice.front(), vertice.back()}, p) ? self::inside_or_onborder : self::outside;
    ...
    while ((cin >> n >> m), n && m) {
        ...
        if (poly1.vertice.size() > 1 && poly2.vertice.size() > 1) {
            for (auto it = poly1.vertice.begin(), itn = it + 1; it != poly1.vertice.end(); ++it, ++itn) {
                ...
                for (auto it2 = poly2.vertice.begin(), it2n = it2 + 1; it2 != poly2.vertice.end(); ++it2, ++it2n) {
                    if (it2n == poly2.vertice.end()) it2n = poly2.vertice.begin();
                    if (is_ins_SS({*it, *itn}, {*it2, *it2n})) run_exec_(cout << RES_Yn[0] << '\n', goto fin__);
                }
            }
        } else {
            if (poly1.vertice.size() == 1 && poly2.vertice.size() == 1) run_exec_(cout << RES_Yn[!is_equal(poly1[0].x, poly2[0].x) || !is_equal(poly1[0].y, poly2[0].y)] << '\n', goto fin__);
            if (poly1.vertice.size() == 1 && poly2.vertice.size() == 2) run_exec_(cout << RES_Yn[!is_on_S({poly2[0], poly2[1]}, poly1[0])] << '\n', goto fin__);
            if (poly1.vertice.size() == 2 && poly2.vertice.size() == 1) run_exec_(cout << RES_Yn[!is_on_S({poly1[0], poly1[1]}, poly2[0])] << '\n', goto fin__);
            if (poly1.vertice.size() == 2 && poly2.vertice.size() == 2) run_exec_(cout << RES_Yn[!is_ins_SS({poly1[0], poly1[1]}, {poly2[0], poly2[1]})] << '\n', goto fin__);
        }
        cout << RES_Yn[1] << '\n';
    fin__:;
    }
