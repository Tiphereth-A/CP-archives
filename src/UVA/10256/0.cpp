...
struct Circle2 {
  ...
  friend size_t insection_type(const Circle2 &c1, const Circle2 &c2) {
...
#undef SCALA_FUNC
  ...
  size_t is_containing_cvh(const point_t &p) const {
    if (vertice.size() == 1)
      return vertice.front() == p ? self::inside_or_onborder : self::outside;
    if (vertice.size() == 2)
      return is_on_S({vertice.front(), vertice.back()}, p) ?
...
auto solve([[maybe_unused]] int t_) -> void {
  ...
  while ((cin >> n >> m), n && m) {
    ...
    foreach_cref_(i, poly1.vertice)
      if (poly2.is_containing_cvh(i))
        run_exec_(cout << RES_Yn[0] << '\n', goto fin__);
    foreach_cref_(i, poly2.vertice)
      if (poly1.is_containing_cvh(i))
        run_exec_(cout << RES_Yn[0] << '\n', goto fin__);
    if (poly1.vertice.size() > 1 && poly2.vertice.size() > 1) {
      for (auto it = poly1.vertice.begin(), itn = it + 1;
           it != poly1.vertice.end();
           ++it, ++itn) {
        if (itn == poly1.vertice.end()) itn = poly1.vertice.begin();
        for (auto it2 = poly2.vertice.begin(), it2n = it2 + 1;
             it2 != poly2.vertice.end();
             ++it2, ++it2n) {
          if (it2n == poly2.vertice.end()) it2n = poly2.vertice.begin();
          if (is_ins_SS({*it, *itn}, {*it2, *it2n}))
            run_exec_(cout << RES_Yn[0] << '\n', goto fin__);
        }
      }
    } else {
      if (poly1.vertice.size() == 1 && poly2.vertice.size() == 1)
        run_exec_(cout << RES_Yn[!is_equal(poly1[0].x, poly2[0].x) ||
                                 !is_equal(poly1[0].y, poly2[0].y)]
                       << '\n',
                  goto fin__);
      if (poly1.vertice.size() == 1 && poly2.vertice.size() == 2)
        run_exec_(cout << RES_Yn[!is_on_S({poly2[0], poly2[1]}, poly1[0])]
                       << '\n',
                  goto fin__);
      if (poly1.vertice.size() == 2 && poly2.vertice.size() == 1)
        run_exec_(cout << RES_Yn[!is_on_S({poly1[0], poly1[1]}, poly2[0])]
                       << '\n',
                  goto fin__);
      if (poly1.vertice.size() == 2 && poly2.vertice.size() == 2)
        run_exec_(
          cout << RES_Yn[!is_ins_SS({poly1[0], poly1[1]}, {poly2[0], poly2[1]})]
               << '\n',
          goto fin__);
    }
    cout << RES_Yn[1] << '\n';
