...
#define SCALA_FUNC(name, func_body)       \
    ...
    inline int quad() const { return std::array<std::array<int, 3>, 3>{std::array<int, 3>{5, 6, 7}, std::array<int, 3>{4, 0, 0}, std::array<int, 3>{3, 2, 1}}[sign(y) + 1][sign(x) + 1]; };
...
struct ConvexHull2: public Polynomial2 {
    ...
    friend inline base coverage_rect_with_min_area(self &lhs) {
        ...
        point_t dir = Line2{lhs[ans_i], lhs[ans_i + 1]}.direction(), vdir = rotate90(dir);
    ...
    inline self &do_convex_cut(const Line2 &l) {
        size_t sz = vs.size();
        cont_t<point_t> cvc;
        for (size_t i = 0; i < sz; ++i) {
    ...
    friend inline self convex_cut(self poly, const Line2 &l) { return poly.do_convex_cut(l); }
};
