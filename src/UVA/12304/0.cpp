...
struct Triangle {
    ...
    point_t A, B, C;
    Triangle() = default;
    inline Triangle(const point_t &a, const point_t &b, const point_t &c):
        A(a), B(b), C(c) {}
...
struct Circle2 {
    ...
    friend inline size_t relation_C(const Circle2 &c1, const Circle2 &c2) {
    ...
    inline size_t relation_P(const point_t &p) const {
...
inline cont_t<Circle2> make_C_rCC(data_t r, const Circle2 &c1, const Circle2 &c2) {
...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    while (cin >> s) {
        ...
        if (s == "TangentLineThroughPoint") {
            ...
            Point2 d1 = Line2{p, tps[0]}.direction(), d2 = Line2{p, tps[1]}.direction();
            ...
            if (is_equal(angle1, angle2)) run_continue_(cout << '[' << angle1 << "]\n");
        ...
        if (s == "CircleThroughAPointAndTangentToALineWithRadius") {
            ...
            if (cs[0].o == cs[1].o) run_continue_(cout << "[(" << cs[0].o.x << ',' << cs[0].o.y << ")]\n");
            cout << "[(" << cs[0].o.x << ',' << cs[0].o.y << "),(" << cs[1].o.x << ',' << cs[1].o.y << ")]\n";
            continue;
        }
        if (s == "CircleTangentToTwoLinesWithRadius") {
            ...
            if (cs.size() == 0) run_continue_(cout << "[]\n");
            sort(all_(cs), [](const Circle2 &lhs, const Circle2 &rhs) { return lhs.o < rhs.o; });
