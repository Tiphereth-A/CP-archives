...
bool is_positive(data_t x) { return !(sign(x) >> 1); }
...
#define SCALA_FUNC(name, func_body) \
  ...
  bool quad() const { return sign(y) + sign(x) >= 1; };
  friend bool quad(const self &lhs) { return lhs.quad(); }
#undef SCALA_FUNC
...
struct Triangle {
  ...
  data_t area() const { return cross(A, B, C) / 2; }
...
struct Line2 {
  ...
  using point_t = Point2;
  point_t l, r;
  Line2(const point_t &_start = point_t{}, const point_t &_end = point_t{})
    : l(_start), r(_end) {}
  friend istream &operator>>(istream &is, self &rhs) {
...
struct Circle2 {
  ...
  point_t o;
  data_t r;
  Circle2(const point_t &c_, data_t r_): o(c_), r(r_) {}
  friend istream &operator>>(istream &is, self &rhs) {
...
string get_s1(char var, double val) {
...
auto solve([[maybe_unused]] int t_) -> void {
  ...
  while (cin >> t) {
    ...
    cout << get_s1('x', o.x) << " + " << get_s1('y', o.y) << " = " << r
         << "^2\n";
    cout << "x^2 + y^2 ";
    double F = o.x * o.x + o.y * o.y - r * r;
    char opx = o.x < 0 ? '+' : '-', opy = o.y < 0 ? '+' : '-',
         opF = F < 0 ? '-' : '+';
    o.x = abs(o.x);
    o.y = abs(o.y);
    F = abs(F);
