...
inline bool is_positive(data_t x) { return !(sign(x) >> 1); }
...
#define SCALA_FUNC(name, func_body)       \
    ...
    inline bool quad() const { return sign(y) + sign(x) >= 1; };
    friend inline bool quad(const self &lhs) { return lhs.quad(); }
#undef SCALA_FUNC
...
struct Triangle {
    ...
    inline data_t area() const { return cross(A, B, C) / 2; }
...
struct Line2 {
    ...
    using point_t = Point2;
    point_t l, r;
    inline Line2(const point_t &_start = point_t{}, const point_t &_end = point_t{}):
        l(_start), r(_end) {}
    ...
    inline bool is_include(const point_t &p) const { return is_positive(cross_mul(direction(), p - l)); }
...
inline bool is_on_S(const Line2 &s1, const Point2 &p) { return cross_sign(s1.l, s1.r, p) == 0 && is_in_middle(s1.l, p, s1.r); }
...
struct Circle2 {
    ...
    point_t o;
    data_t r;
    inline Circle2(const point_t &c_, data_t r_):
        o(c_), r(r_) {}
...
inline string get_s1(char var, double val) {
    if (is_zero(val)) return string{var} + "^2";
    char op = is_positive(val) ? '-' : '+';
    stringstream ss;
    ss << fixed << setprecision(3) << abs(val);
    string ret;
    ss >> ret;
    return string{'('} + var + ' ' + op + ' ' + ret + ")^2";
}
inline auto solve([[maybe_unused]] int t_) -> void {
    Triangle t;
    while (cin >> t) {
        Point2 o = t.center_O();
        double r = dist_PP(o, t.A);
        cout << get_s1('x', o.x) << " + " << get_s1('y', o.y) << " = " << r << "^2\n";
        cout << "x^2 + y^2 ";
        double F = o.x * o.x + o.y * o.y - r * r;
