...
struct Line2 {
    ...
    point_t l, r;
    inline Line2(const point_t &_start, const point_t &_end):
        l(_start), r(_end) {}
...
inline Point2 dist_PPS_P(const Line2 &s1, const Point2 &p) {
    if (s1.l == s1.r) return s1.l;
    Point2 h = proj(s1, p);
    if (is_in_middle(s1.l, h, s1.r)) return h;
    return is_less(dist_PP(s1.l, p), dist_PP(s1.r, p)) ? s1.l : s1.r;
}
...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    while (cin >> x >> y) {
        ...
        Point2 ansP;
        foreach_cref_(i, l) {
            auto pp = dist_PPS_P(i, m);
            double d = dist_PP(pp, m);
            if (is_less(d, ans)) {
                ans = d;
                ansP = pp;
            }
