...
inline bool is_less(data_t lhs, data_t rhs) { return is_negative(lhs - rhs) & 2; }
...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    for_(i, 1, n) {
        ...
        ans += w * h;
        Point2 c{x, y};
        v.emplace_back(c + Point2{w / 2, h / 2}.do_rotate(phi));
        v.emplace_back(c + Point2{-w / 2, h / 2}.do_rotate(phi));
        v.emplace_back(c + Point2{w / 2, -h / 2}.do_rotate(phi));
        v.emplace_back(c + Point2{-w / 2, -h / 2}.do_rotate(phi));
    }
    cout << ans * 100 / Polynomial2{v}.do_convex_hull().area() << " %\n";
}
int main() {
