...
int n, m, minn, t;
inline int _f(int x, int y, int pos) {
    ...
    while (1) {
        ...
        for (int i = 1; i <= n; ++i)
            for (int j = 1; j < m; ++j) {
                ...
                minn = min(minn, val);
                if (i == 1)
                    addEdge(_f(i, j, 2), 2 * n * m - 2 * n - 2 * m + 4, val);
                else if (i == n)
                    addEdge(2 * n * m - 2 * n - 2 * m + 3, _f(i - 1, j, 1), val);
                else
                    addEdge(_f(i - 1, j, 1), _f(i, j, 2), val), addEdge(_f(i, j, 2), _f(i - 1, j, 1), val);
            }
        for (int i = 1; i < n; ++i)
            for (int j = 1; j <= m; ++j) {
                val = read();
                minn = min(minn, val);
                if (j == 1)
                    addEdge(2 * n * m - 2 * n - 2 * m + 3, _f(i, j, 1), val);
                else if (j == m)
                    addEdge(_f(i, j - 1, 2), 2 * n * m - 2 * n - 2 * m + 4, val);
                else
                    addEdge(_f(i, j - 1, 2), _f(i, j, 1), val), addEdge(_f(i, j, 1), _f(i, j - 1, 2), val);
            }
        for (int i = 1; i < n; ++i)
            for (int j = 1; j < m; ++j) {
                val = read();
                minn = min(minn, val);
                addEdge(_f(i, j, 1), _f(i, j, 2), val), addEdge(_f(i, j, 2), _f(i, j, 1), val);
            }
        if (n == 1 || m == 1) {
            printf("%d\n", minn);
            return 0;
