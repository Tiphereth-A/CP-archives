...
i64 choose(i64 n, i64 m, const i64 &mod = MOD) { return fact[n] * inv_fact[m] % mod * inv_fact[n - m] % mod; }
int main() {
    int n, k;
    cin >> n >> k;
    ...
    i64 ans = 0;
    for (int i = k; i <= n; ++i) ans = (ans + ((i + k) % 2 ? -1 : 1) * choose(n, i) * choose(i, k) % MOD * (ppow[n - i] + MOD - 1) % MOD + MOD) % MOD;
    cout << ans << endl;
    return 0;
}
