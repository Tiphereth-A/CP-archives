...
#include <vector>
using namespace std;
#define for_(i, l, r, vars...) for (ptrdiff_t i = (l), i##end = (r), ##vars; i <= i##end; ++i)
#define read_var_(type, name) \
    type name;                \
    std::cin >> name
#define read_container_(type, name, size) \
    type name(size);                      \
    for_(i, 0, (size)-1) std::cin >> name[i];
...
#define SCALA_FUNC(name, func_body)       \
    ...
    inline int quad() const { return ((int[9]){5, 6, 7, 4, 0, 0, 3, 2, 1})[(sign(y) + 1) * 3 + sign(x) + 1]; }
...
#undef SCALA_FUNC
    ...
    inline uint_data_t cnt_intP_border() {
        uint_data_t ret = 0;
        for (size_t i = 0; i < vs.size(); ++i) ret += gcd(f2uint(abs(vs[i].x - vs[next(i)].x)), f2uint(abs(vs[i].y - vs[next(i)].y)));
        return ret;
    }
...
struct ConvexHull2: public Polynomial2 {
    ...
    explicit ConvexHull2(size_t size_):
        Polynomial2(size_) {}
    explicit ConvexHull2(const std::vector<Point2> &vs_, bool inited = false, bool strict = true):
        Polynomial2(vs_) {
    ...
    friend inline bool is_ins_cvh(const ConvexHull2 &cvhl, const ConvexHull2 &cvhr) {
        for (std::vector<Point2>::const_iterator it = cvhl.vs.begin(); it != cvhl.vs.end(); ++it)
            if (cvhr.relation_P(*it)) return false;
        for (std::vector<Point2>::const_iterator it = cvhr.vs.begin(); it != cvhr.vs.end(); ++it)
        ...
        if (n > 1 && m > 1) {
            for (size_t l = 0, nextl = 1; l < n; nextl = cvhl.next(l = nextl))
                for (size_t r = 0, nextr = 1; r < m; nextr = cvhl.next(r = nextr))
                    if (is_ins_SS(Line2(cvhl[l], cvhl[nextl]), Line2(cvhr[r], cvhr[nextr]))) return false;
        } else if (n == 1) {
    ...
    friend inline Polynomial2 coverage_rect_with_min_area(ConvexHull2 &lhs) {
        ...
        if (sz == 0) return lhs;
        if (sz == 1) {
            std::vector<Point2> __;
            __.push_back(lhs[0]);
            __.push_back(lhs[0]);
            __.push_back(lhs[0]);
            __.push_back(lhs[0]);
            return Polynomial2(__);
        }
        if (sz == 2) {
            std::vector<Point2> __;
            __.push_back(lhs[0]);
        ...
        std::vector<Point2> __;
        __.push_back(ins_LL(li, lp));
        __.push_back(ins_LL(lp, lr));
        __.push_back(ins_LL(lr, lq));
        __.push_back(ins_LL(lq, li));
        Polynomial2 ret(__);
        lhs.vs.pop_back();
        return ret;
    }
...
inline bool ins_hP_check(const Line2 &u, const Line2 &v, const Line2 &w) { return w.is_include(ins_LL(u, v)); }
inline ConvexHull2 ins_hP(std::vector<Line2> vl) {
    ...
    for (std::vector<Line2>::iterator it = vl.begin(); it != vl.end(); ++it) {
        ...
        while (q.size() > 1 && !ins_hP_check(*(q.rbegin() + 1), q.back(), *it)) q.pop_back();
        while (q.size() > 1 && !ins_hP_check(*(q.begin() + 1), q.front(), *it)) q.pop_front();
        q.push_back(*it);
    }
    while (q.size() > 2 && !ins_hP_check(*(q.rbegin() + 1), q.back(), q.front())) q.pop_back();
    while (q.size() > 2 && !ins_hP_check(*(q.begin() + 1), q.front(), q.back())) q.pop_front();
...
inline void solve(int t_) {
    read_var_(int, n);
    read_container_(vector<Point2>, vps, n);
    vector<Line2> vl;
    for_(i, 0, n - 1) vl.push_back(Line2(vps[(i + 1) % n], vps[i]));
    cout << ins_hP(vl).area() << '\n';
}
int main() {
