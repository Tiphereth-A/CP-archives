...
double dp(int state, int now) {
    ...
    double ret = bicycles[now].second * dist[now][n] / t + (1 - bicycles[now].second) * dist[now][n] / r;
    _for(i, 0, k - 1) {
        if (i == now) continue;
        if (state & (1 << i)) continue;
        ret = min(ret, bicycles[now].second * (dp(state | (1 << i), i) + 1.0 * dist[now][bicycles[i].first] / t) + (1 - bicycles[now].second) * dist[now][n] / r);
    }
