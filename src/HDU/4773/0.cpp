...
#define SCALA_FUNC(name, ...)               \
    inline data_t name() const __VA_ARGS__; \
    ...
    inline int quad() const { return QUAD__[(sign(y) + 1) * 3 + sign(x) + 1]; }
    friend inline int quad(const Point2 &lhs) { return lhs.quad(); }
#undef SCALA_FUNC
#define SELF_FUNC(name, ...)                \
    inline Point2 &do_##name() __VA_ARGS__; \
...
#define SELF_FUNC1(name, rhs_type, rhs, ...)                   \
    inline Point2 &do_##name(const rhs_type &rhs) __VA_ARGS__; \
    friend inline Point2 name(Point2 lhs, const rhs_type &rhs) { return lhs.do_##name(rhs); }
...
inline data_t ang2pi_PP(const Point2 &p1, const Point2 &p2) {
...
struct Line2 {
    ...
    inline Line2 &do_push(data_t dist) {
    ...
    friend inline Line2 push(Line2 line, data_t dist) { return line.do_push(dist); }
};
...
struct Circle2 {
    ...
    inline data_t arc(data_t angle = PI * 2) const { return angle * r; }
...
inline Circle2 make_C_PPP(const Point2 &p1, const Point2 &p2, const Point2 &p3) {
    Point2 o = Triangle(p1, p2, p3).center_O();
    return Circle2(o, dist_PP(o, p1));
}
inline Point2 inv_P2P(const Circle2 &c, const Point2 &p) {
    Point2 v = p - c.o;
...
inline Line2 inv_C2L(const Circle2 &c, const Circle2 &c2) {
    Point2 v = (c2.o - c.o).do_rot90();
    return Line2(inv_P2P(c, c2.o + v), inv_P2P(c, c2.o - v));
}
inline Circle2 inv_L2C(const Circle2 &c, const Line2 &l) {
