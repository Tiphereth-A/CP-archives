...
#undef v
    ...
    vecu vistime(vp.size()), resfdel(vp.size()), resfnew(vp.size()),
      resptid(vp.size());
    auto horizon = [&](auto &&rec, u32 f, point3d<FP> CR p) -> u32 {
      if (relation_PlP(faces[f].p, p) != above_plp) return 0;
      if (faces[f].vistime == tm) return -1_u32;
      faces[f].vistime = tm, faces[f].isdel = 1, resfdel.push_back(faces[f].id);
      u32 ret = -2_u32;
      flt_(u32, i, 0, 3) if (u32 res = rec(rec, faces[f].n[i], p); !res) {
        u32 pt[2]{faces[f].pid[i], faces[f].pid[(i + 1) % 3]};
        flt_(u32, j, 0, 2) if (vistime[pt[j]] != tm)
          vistime[pt[j]] = tm,
          e1[pt[j]].netid = pt[(j + 1) % 2], e1[pt[j]].facetid = faces[f].n[i];
        else e2[pt[j]].netid = pt[(j + 1) % 2],
             e2[pt[j]].facetid = faces[f].n[i];
        ret = pt[0];
      }
    ...
    while (!que.empty()) {
      ...
      while (vistime[s] != tm) {
        ...
        faces[fnew = faces.size() - 1].id = faces.size() - 1;
...
int main() {
  ...
  cvh cvh(vp);
  std::cout << std::fixed << std::setprecision(3) << cvh.surface_area() << '\n';
  return 0;
}
