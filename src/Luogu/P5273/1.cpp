...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  cin >> sk;
  i64 k = 0, km = 0, _ = 0;
  for (char ch : sk) ((k *= 10) += ch - '0') %= MOD;
  for (char ch : sk) ((km *= 10) += ch - '0') %= MOD - 1;
  for_(i, 0, min(6, (int)sk.size() - 1)) (_ *= 10) += sk[i] - '0';
  poly f(n);
  cin >> f;
  int c0 = 0;
  for (; c0 < n; ++c0)
    if (f[c0]) break;
  if (f[0] == 0 && (_ >= n || c0 * k >= n)) f = poly(n);
  else {
    f.do_shl(c0);
    i64 inv__ = tifa_libs::poly::detail__::inverse(f[0], MOD),
        pow__ = tifa_libs::poly::detail__::qpow(f[0], km, MOD);
    (f *= inv__).do_pow(k) *= pow__;
    f.do_shr(c0 * k);
  }
  cout << f << '\n';
}
