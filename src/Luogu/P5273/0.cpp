...
public:
  ...
  Poly &shift_left(size_t offset) {
    ...
    std::move(data.begin() + offset, data.end(), data.begin());
    data.erase(data.end() - offset, data.end());
    return *this;
  }
  Poly &shift_right(size_t offset) {
    ...
    std::fill(data.begin(), data.begin() + offset, 0);
    return *this;
  }
...
#define FUNCP2_(name, type1, var1, type2, var2, ...)                    \
  ...
  FUNCP2_(pow, ludata_t, y, ludata_t, y_mod_phiMOD, {
    ...
    shift_left(k_);
    data.resize(sz_ - k_ * y);
    data_t c_ = data[0], inv_c_ = qpow(c_), c_y_ = qpow(c_, y_mod_phiMOD);
    *this *= inv_c_;
    *this = (log(*this) * y).do_exp() * c_y_;
    shift_right(k_ * y);
    return *this;
  })
#undef FUNC_
...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  Poly px(n);
  cin >> px;
  int64_t k = dec2uint_mod(kk, Polynomial::MOD);
  bool flag = f__;
  f__ = 0;
  int64_t k2 = dec2uint_mod(kk, Polynomial::MOD - 1);
  cout << px.do_pow(k + (flag ? Polynomial::MOD : 0),
                    k2 + (f__ ? Polynomial::MOD - 1 : 0));
}
int main() {
