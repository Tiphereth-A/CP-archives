...
struct Triangle {
    ...
    bool is_obtuse() const { return is_negative(dot(A, B, C)) || is_negative(dot(B, C, A)) || is_negative(dot(C, A, B)); }
...
constexpr data_t dist_SS(const Line2 &s1, const Line2 &s2) {
    ...
    return std::min({dist_PPS(s1, s2.l), dist_PPS(s1, s2.r), dist_PPS(s2, s1.l), dist_PPS(s2, s1.r)});
}
struct Polynomial2 {
    ...
    explicit Polynomial2(size_t size_):
        vertice(size_) {}
    Polynomial2(const cont_t<point_t> &vertice_):
        vertice(vertice_) {}
...
#undef SCALA_FUNC
    ...
    inline size_t is_containing_cvh(const point_t &p) const {
        if (is_positive(cross(vertice.front(), p, vertice[1])) || is_positive(cross(vertice.front(), vertice.back(), p))) return self::outside;
        auto it = std::lower_bound(vertice.begin() + 1, vertice.end(), p, [&](const point_t &lhs, const point_t &rhs) { return is_positive(cross(vertice.front(), lhs, rhs)); }) - 1;
        auto next_it = it + 1;
        ...
        return !res && !is_positive(dot(p, *it, *next_it)) ? self::inside_or_onborder : self::outside;
    }
#define SELF_FUNC(name, func_body)      \
...
#undef SELF_FUNC
    inline self &do_minkowski_sum_cvh(const self &rhs) {
        ...
        vertice = result;
        return this->do_convex_hull();
    }
    friend inline self minkowski_sum_cvh(self poly, const self &rhs) { return poly.do_minkowski_sum_cvh(rhs); }
...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    for_(i, 1, n - 1, nexti) {
        ...
        if (!is_zero(a))
            vl.emplace_back(-c / a, 0, b - c / a, -a);
        else if (!is_zero(b))
            vl.emplace_back(0, -c / b, b, -a - c / b);
    }
    cout << ins_hP(vl).area() / Polynomial2{vp}.area();
}
int main() {
