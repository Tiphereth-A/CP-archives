...
bool euler_path(int n, int m, int default_start, tuple<int, int, string> *arcs, const int *in_deg, const int *out_deg) {
    ...
    sort(arcs + 1, arcs + m + 1, [](const auto &lhs, const auto &rhs) -> bool { return get<2>(lhs) > get<2>(rhs); });
    for (int i = 1; i <= m; ++i) addEdge(get<0>(arcs[i]), get<1>(arcs[i]), get<2>(arcs[i]));
    epath_cnt = m;
    dfs(start);
    return true;
}
