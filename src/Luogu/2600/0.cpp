...
constexpr data_t dist_PP(const Point2 &lhs, const Point2 &rhs) { return hypot(lhs.x - rhs.x, lhs.y - rhs.y); }
...
data_t min_dist_Ps(const cont_t<Point2> &vp, ptrdiff_t l, ptrdiff_t r) {
    ...
    std::sort(q.begin(), q.end(), [](const Point2 &lhs, const Point2 &rhs) -> bool { return lhs.y < rhs.y; });
...
inline data_t area_PolyC(const Polynomial2 &poly, const Circle2 &c) {
    ...
    if (sz < 3) return ans;
    for (size_t i = 0, j = 1; i < sz; i = j, j = poly.next(j)) ans += -(int)(cross_sign(c.o, poly[j], poly[i]) < 0) * area_CT(c.r, poly[i] - c.o, poly[j] - c.o);
    return fabs(ans);
}
struct ConvexHull2: public Polynomial2 {
    ...
    inline size_t relation_P(const Point2 &p) const override {
        if (vs.size() == 1) return vs.front() == p ? inside_or_onborder : Polynomial2::outside;
        if (vs.size() == 2) return is_on_S({vs.front(), vs.back()}, p) ? inside_or_onborder : Polynomial2::outside;
        if (is_positive(cross(vs.front(), p, vs[1])) || is_positive(cross(vs.front(), vs.back(), p))) return Polynomial2::outside;
        ...
        return !res && !is_positive(dot(p, *it, *next_it)) ? inside_or_onborder : Polynomial2::outside;
    }
    ...
    inline data_t convex_diameter() const {
        size_t sz = vs.size();
...
inline auto solve([[maybe_unused]] int t_) -> void {
    read_var_(int, n);
    read_container_(vc<int>, x, n);
    read_container_(vc<int>, y, n);
    vc<Line2> vl;
    ...
    for_(i, 0, n - 2) vl.emplace_back(x[i], y[i], x[i + 1], y[i + 1]);
    ConvexHull2 cvh = ins_hP(vl);
    double ans = DBL_MAX;
    sort(all_(cvh.vs), [](const auto &lhs, const auto &rhs) { return lhs.x < rhs.x; });
    size_t i = 0, j = 0;
    while (i < cvh.vs.size() && j < n) {
        if (i && is_equal(cvh[i].x, cvh[i - 1].x) && is_greater(cvh[i].y, cvh[i - 1].y)) run_continue_(++i);
        if (cvh[i].x > x[j]) {
            Point2 temp = ins_LL({cvh[i], cvh[i - 1]}, {{x[j], y[j]}, {x[j], y[j] + 1}});
            chkmin(ans, abs(temp.y - y[j++]));
        } else {
            Point2 temp = ins_LL({{x[j - 1], y[j - 1]}, {x[j], y[j]}}, {cvh[i], {cvh[i].x, cvh[i].y + 1}});
            chkmin(ans, abs(cvh[i++].y - temp.y));
        }
    }
    cout << ans;
}
