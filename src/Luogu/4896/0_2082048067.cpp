...
int main() {
    ...
    for (int l = 0; l < n; ++l) {
        ...
        if (s2 == "left!" && s3 == "" && teaID.count(s1)) {
            _teaout(teaID[s1]);
        }
        if (s2 == "started" && s3 == "playing" && s4 == "games!" && s5 == "" && stuID.count(s1)) {
            _stuin(stuID[s1]);
            if (onlineT) _killstu;
        }
        if (s2 == "stopped" && s3 == "playing" && s4 == "games!" && s5 == "" && stuID.count(s1)) {
            _stuout(stuID[s1]);
        }
        if (deadS == '\037') break;
    }
