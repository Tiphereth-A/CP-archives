...
void modify(int pre, int l, int r, int &now, int p) {
    ...
    int mid = _mid(l, r);
    if (p <= mid)
        modify(seq[pre].l, l, mid, seq[now].l, p);
    else
        modify(seq[pre].r, mid + 1, r, seq[now].r, p);
}
int query(int node_l, int node_r, int l, int r, int k) {
    if (l == r) return l;
    int mid = _mid(l, r), tmp = seq[seq[node_r].l].val - seq[seq[node_l].l].val;
    if (k <= tmp)
        return query(seq[node_l].l, seq[node_r].l, l, mid, k);
    else
        return query(seq[node_l].r, seq[node_r].r, mid + 1, r, k - tmp);
}
int main() {
    scanf("%d%d", &n, &m);
    _for(i, 1, n) {
