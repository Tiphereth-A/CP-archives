...
int main() {
    ...
    for (int __m, i = 0; i < n; ++i)
        for (int j = 0; j < m; ++j) {
            cin >> __m;
            if (__m) maps[i][j] = maps[i][j + 1] = maps[i + 1][j] = maps[i + 1][j + 1] = -1;
        }
    ...
    q.push({start_point, 0, ((dir == 'E') ? 0 : ((dir == 'S') ? 1 : ((dir == 'W') ? 2 : 3)))});
        ...
        for (int dir = 0; dir < 4; ++dir)
            for (int i = 0; i < 3; ++i) {
                ...
                if (xx > 0 && xx < n && yy > 0 && yy < m) {
                    ...
                    int tmp_step = now.step + std::min(4 - std::abs(dir - now.dir), std::abs(dir - now.dir)) + 1;
    ...
    cout << (maps[end_point.x][end_point.y] > 0 ? maps[end_point.x][end_point.y] : -1);
}