...
CEXP void zt_supset(vec<T> &f) {
  ...
  for (u32 i = 1; i < n; i *= 2)
    flt_(u32, j, 0, n) if (!(j & i)) f[j] += f[j | i];
}
template <class T>
...
CEXP void zt_subset(vec<T> &f) {
  ...
  assert(std::has_single_bit(n));
  for (u32 i = 1; i < n; i *= 2)
    flt_(u32, j, 0, n) if (!(j & i)) f[j | i] += f[j];
}
template <class T>
...
namespace tifa_libs::math {
template <class T>
CEXP vec<T> conv_or(vec<T> l, vec<T> r) {
...
CEXP void wht(vec<T> &f, u32 n = 0) {
  if (!n) n = (u32)f.size();
  for (u32 i = 1; i < n; i *= 2) flt_(u32, j, 0, n) if (!(j & i)) {
...
int main() {
  ...
  std::cin >> a >> b;
  std::cout << tifa_libs::math::conv_or(a, b) << '\n';
