...
struct Line2 {
    ...
    using point_t = Point2;
    point_t l, r;
    constexpr Line2(const point_t &_start = point_t{}, const point_t &_end = point_t{}):
        l(_start), r(_end) {}
    ...
    constexpr bool is_include(const point_t &p) const { return is_positive(cross_mul(direction(), p - l)); }
...
constexpr bool is_on_S(const Line2 &s1, const Point2 &p) { return cross_sign(s1.l, s1.r, p) == 0 && is_in_middle(s1.l, p, s1.r); }
...
struct Circle2 {
    ...
    point_t o;
    data_t r;
    constexpr Circle2(const point_t &c_, data_t r_):
        o(c_), r(r_) {}
...
data_t min_dist(const cont_t<Point2> &poly, size_t l, size_t r) {
    ...
    if (r - l <= 5) {
        for (size_t i = l; i < r; ++i)
            for (size_t j = l; j < i; ++j) ret = std::min(ret, dist_PP(poly[i], poly[j]));
        return ret;
    }
    size_t mid = r - (r - l) / 2;
...
inline Polynomial2 ins_hP(cont_t<Line2> l) {
...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    read_var_(double, a);
    read_var_(double, r);
    Point2 d[4] = {{a / 2 - r, b / 2 - r}, {-a / 2 + r, b / 2 - r}, {a / 2 - r, -b / 2 + r}, {-a / 2 + r, -b / 2 + r}};
    vector<Point2> vp;
    vp.reserve(n);
    double x, y, theta;
    for_(i, 1, n) {
        cin >> x >> y >> theta;
        Point2 c{x, y};
        foreach_cref_(j, d) vp.push_back(c + Point2{j}.do_rotate(theta));
    }
    cout << Polynomial2{vp}.do_convex_hull().circum() + r * PI * 2;
}
int main() {
