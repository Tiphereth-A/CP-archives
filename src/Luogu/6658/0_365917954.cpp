...
#define _for_graph(head, e, i, now) for (int i = head[now], v = e[i].to; i; v = e[i = e[i].next].to)
namespace union_find {
int par[N];
inline void init() { memset(par, 0xff, sizeof par); }
...
inline bool cmp(const vector<int>& x, const vector<int>& y) { return x[0] < y[0]; }
void solve() {
