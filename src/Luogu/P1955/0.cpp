...
constexpr auto subtuple(Tuple &&t) {
  static_assert(Begin <= TPL_SIZE_(Tuple) && Len <= TPL_SIZE_(Tuple) &&
                ...
                "Out of range");
  return tuple_deta_::subtuple_impl_<Begin>(t, std::make_index_sequence<Len>());
}
...
constexpr auto apply2(BinOp &&f, Tuple &&lhs, Tuple &&rhs) {
  return tuple_deta_::apply2_impl_(
...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  iota(fa, fa + b.size() + 1, 0);
  bool f = 1;
  for (auto &&[x, y, op] : a) {
    auto &&fx = find(lower_bound(b.begin(), b.end(), x) - b.begin()),
         fy = find(lower_bound(b.begin(), b.end(), y) - b.begin());
    if (op == 1) run_continue_(fa[fx] = fy);
    if (fx == fy) run_break_(f = 0);
  }
  cout << RES_YN[f] << '\n';
}
int main() {
