...
#define _mid(l, r) (((l) + (r)) >> 1)
    ...
    Node data[N << 2];
    data_type tag[N << 2];
    inline void _push_up(int p) { data[p] = data[_lch(p)] + data[_rch(p)]; }
    inline void _push_down(int p, int len) {
        if (!tag[p]) return;
        data[_lch(p)] = {data[_lch(p)].a + tag[p] * (len - len / 2), data[_lch(p)].a_sqr + data[_lch(p)].a * 2 * tag[p] + tag[p] * tag[p] * (len - len / 2)};
        data[_rch(p)] = {data[_rch(p)].a + tag[p] * (len / 2), data[_rch(p)].a_sqr + data[_rch(p)].a * 2 * tag[p] + tag[p] * tag[p] * (len / 2)};
        tag[_lch(p)] += tag[p];
        tag[_rch(p)] += tag[p];
        tag[p] = 0;
    }
    ...
    void _update(data_type k, int L, int R, int p, int l, int r) {
        if (L <= l && r <= R) {
            data[p] = {data[p].a + k * (r - l + 1), data[p].a_sqr + data[p].a * 2 * k + k * k * (r - l + 1)};
            tag[p] += k;
            return;
        }
        _push_down(p, r - l + 1);
  ...
  public:
    ...
    data_type query(int L, int R, int type) { return _query(L, R, type, 1, 1, n); }
    void debug() {
        _for(i, 1, n << 2) std::cerr << data[i].a << " " << data[i].a_sqr << " " << tag[i] << std::endl;
        std::cerr << std::endl;
    }
};
Segtree<N> tr;
int main() {
    ...
    while (m--) {
        ...
        switch (op) {
            ...
            case 3:
                double x = tr.query(l, r, 3) / (r - l + 1), y = tr.query(l, r, 2) / (r - l + 1);
                cout << x - y * y << '\n';
                break;
        }
    }
