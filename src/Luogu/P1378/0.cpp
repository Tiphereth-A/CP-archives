...
using namespace std;
constexpr double dist(pii const &a, pii const &b) {
...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  pii p1, p2;
  cin >> p1 >> p2;
  int l = min(p1.first, p2.first), r = max(p1.first, p2.first),
      u = max(p1.second, p2.second), d = min(p1.second, p2.second),
      s = (r - l) * (u - d);
  vc<pii> vp(n);
  for (auto &i : vp) cin >> i;
  vc<int> idx(n);
  ...
  int ans = 1e9;
  do
    chkmin(ans, (int)round(s - [&]() -> double {
             double ret = 0;
             vc<double> rr(n);
             for_(i, 0, n - 1) {
               double R = 1e9;
               rfor_(j, i - 1, 0)
                 chkmin(R, dist(vp[idx[i]], vp[idx[j]]) - rr[j]);
               if (R < 0) continue;
               ret += (rr[i] = min({(double)(u - vp[idx[i]].second),
                                    (double)(vp[idx[i]].second - d),
                                    (double)(vp[idx[i]].first - l),
                                    (double)(r - vp[idx[i]].first),
                                    R})) *
                      rr[i] * PI;
             }
             return ret;
           }()));
  while (next_permutation(all_(idx)));
  cout << ans << '\n';
}
int main() {
