...
CEXP auto steiner_tree(G CR g, vecu CR x, T INF = inf_v<T>) NE {
  const u32 n = (u32)g.size(), k = (u32)x.size();
  vvec<T> dp(1 << k, vecu(n, INF));
  flt_(u32, i, 0, k) dp[1 << i][x[i]] = 0;
  ds::rheap<T, u32> q;
  flt_(u32, s, 1, 1_u32 << k) {
    ...
    while (!q.empty()) {
      auto [dis, u] = q.top();
      if (q.pop(); dp[s][u] != dis) continue;
      for (auto [v, w] : g[u])
        if (dp[s][v] > dis + w) q.emplace(dp[s][v] = dis + w, v);
    }
  }
  if CEXP (get_edges) {
    vecptu edges;
    return std::make_pair(dp.back()[x[0]], uniq(edges));
  } else return dp.back()[x[0]];
}
}  // namespace tifa_libs::graph
namespace tifa_libs::graph {
