...
protected:
  query_func qfunc;
  std::vector<std::vector<data_t>> f;

public:
  RMQ_ST() = delete;
  RMQ_ST(query_func qfunc): qfunc(qfunc), f() {}
  template <typename init_func = std::function<data_t(size_t)>>
  void init(size_t n, init_func ifunc) {
    const size_t K = (size_t)std::log2(n) + 1;
    f.resize(K);
    for (auto &i : f) i.resize(n);
  ...
  data_t query(size_t l, size_t r) const {
    size_t _ = (size_t)std::log2(r - l);
    return qfunc(f[_][l], f[_][r - (1 << _)]);
  }
};
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  RMQ_ST<int> st([](int x, int y) { return max(x, y); });
  st.init(n, [&](size_t x) { return a[x]; });
  for_(i, 1, m, l, r) {
    cin >> l >> r;
    cout << st.query(l - 1, r) << '\n';
  }
