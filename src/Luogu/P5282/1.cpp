...
namespace tifa_libs::math {
template <class T>
inline poly<T> poly_ctsh(poly<T> const &f,
...
template <class T>
inline poly<T>
poly_ctsh(poly<T> const &f, typename T::value_type c, usz m = 0) {
  usz n = f.size();
...
inline vec<mint>
conv_mtt(vec<mint> const &l, vec<mint> const &r, size_t ans_size) {
...
template <class mint, class FP = double>
inline vec<mint> conv_mtt(vec<mint> const &l, vec<mint> const &r) {
