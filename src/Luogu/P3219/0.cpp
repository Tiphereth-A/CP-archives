...
private:
  ...
  double asr(double l, double r, double area) const {
    double mid = r - (r - l) * .5;
    ...
    if (std::abs(ls + rs - area) <= EPS) return area;
...
public:
  ...
  double operator()(double l, double r, double eps) const {
    return asr(l, r, simpson(l, r, f(l), f(r - (r - l) * .5), f(r)));
  }
};
struct Node {
  int x, y, m;
...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  sort(all_(v_), [](const Node &l, const Node &r) { return l.m > r.m; });
  v.push_back(v_.front());
  for_(i, 1, n - 1) {
    auto &&now = v_[i];
    if (now.m == 0) break;
    if (none_of(all_(v), [&now](const Node &x) {
          return now.x >= x.x && now.x + now.m <= x.x + x.m && now.y >= x.y &&
                 now.y + now.m <= x.y + x.m &&
                 now.x + now.y + now.m <= x.x + x.y + x.m;
        }))
      v.push_back(now);
  }
  vc<pair<long double, long double>> seg;
  seg.reserve(n);
  auto f = [&v, &seg](long double pos) -> long double {
    seg.clear();
    for (auto [x, y, m] : v)
      if (x < pos && pos < x + m) seg.emplace_back(y, x + y + m - pos);
    if (seg.empty()) return 0;
    sort(all_(seg));
    long double ans = 0, lst = seg.front().first;
    for (auto &&[l, r] : seg)
      if (r - lst > 0) {
        ans += r - max(l, lst);
        lst = r;
  ...
  long double ans = 0;
  SimpsonSolver ss(f);
  for_(i, 1, pos.size() - 1) ans += ss(pos[i - 1] + EPS, pos[i] - 2 * EPS, EPS);
