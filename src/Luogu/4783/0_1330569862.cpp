...
template <typename Tp, Tp mod, _TRAITS(std::is_integral<Tp>::value && (1 <= mod))>
class mod_integer {
  public:
  ...
  public:
    ...
    template <typename Up, _TRAITS(std::is_integral<Up>::value&& std::is_convertible<Up, data_t>::value)>
    mod_integer(const Up& _num) noexcept : num(static_cast<data_t>(_num)) { this->safe_mod(); }
    template <typename Up, _TRAITS(std::is_convertible<Up, self&>::value)>
    mod_integer(Up&& rhs) noexcept : num(std::forward<self>(rhs).data()) {}
    template <typename Up, _TRAITS(std::is_integral<Up>::value&& std::is_convertible<Up, data_t>::value)>
    inline self& operator=(const Up& _num) noexcept {
    ...
    inline constexpr self& transform_unary_raw(std::function<data_t(const data_t&)>&& op) {
        this->data() = op(this->data());
        return *this;
    }
    inline constexpr self& transform_unary(std::function<data_t(const data_t&)>&& op) { return this->transform_unary_raw(std::move(op)).safe_mod(); }
    inline constexpr self& transform_binary_raw(const self& rhs, std::function<data_t(const data_t&, const data_t&)>&& op) {
        this->data() = op(this->data(), rhs.data());
        return *this;
    }
    inline constexpr self& transform_binary(const self& rhs, std::function<data_t(const data_t&, const data_t&)>&& op) { return this->transform_binary_raw(rhs, std::move(op)).safe_mod(); }
    friend inline constexpr self calc_unary_raw(const self& lhs, std::function<data_t(const data_t&)>&& op) {
        self ret(lhs);
        return ret.transform_unary_raw(op);
    }
    friend inline constexpr self calc_unary(const self& lhs, std::function<data_t(const data_t&)>&& op) { return calc_unary_raw(lhs, std::move(op)).safe_mod(); }
    friend inline constexpr self calc_binary_raw(const self& lhs, const self& rhs, std::function<data_t(const data_t&, const data_t&)>&& op) {
        self ret(lhs);
        return ret.transform_binary_raw(rhs, op);
    }
    friend inline constexpr self calc_binary(const self& lhs, const self& rhs, std::function<data_t(const data_t&, const data_t&)>&& op) { return calc_binary_raw(lhs, rhs, std::move(op)).safe_mod(); }
...
#define _square_matrix_needed \
  ...
  public:
    matrix(const std::size_t& _row, const std::size_t& _col, const Equal& _equal = Equal()) : row(_row), col(_col), mat(_row, std::vector<Tp>(_col)), equ(_equal) {}
    template <typename Up, _TRAITS(_CONVERTIBLE(Up, Tp))>
    matrix(const std::size_t& _row, const std::size_t& _col, Up&& scalar, const Equal& _equal = Equal()) : row(_row), col(_col), mat(_row, std::vector<Tp>(_col, scalar)), equ(_equal) {}
    template <typename Up, _TRAITS(_CONVERTIBLE(Up, self&))>
    matrix(Up&& rhs) : row(std::forward<self>(rhs).get_row()), col(std::forward<self>(rhs).get_col()), mat(std::forward<self>(rhs).mat), equ(std::forward<self>(rhs).equ) {}
    template <typename Up, _TRAITS(_CONVERTIBLE(Up, self))>
    ...
    inline constexpr Tp& data(const size_t& r, const size_t& c) { return this->mat[r][c]; }
    inline constexpr self trans() {
        self ret(this->get_col(), this->get_row(), this->equ);
        _for_each(i, j) ret.data(j, i) = this->data(i, j);
        return ret;
    }
    inline constexpr std::size_t rank() { return gauss(self(*this)); }
    inline constexpr Tp det() const {
    ...
    inline constexpr self& add(const Tp& scalar) {
        _for_each(i, j) this->data(i, j) += scalar;
        return *this;
    }
    inline constexpr self& minus(const Tp& scalar) {
        _for_each(i, j) this->data(i, j) -= scalar;
        return *this;
    }
    inline constexpr self& multiply(const Tp& scalar) {
        _for_each(i, j) this->data(i, j) *= scalar;
        return *this;
    }
    inline constexpr self& divide(const Tp& scalar) {
        _for_each(i, j) this->data(i, j) /= scalar;
        return *this;
    }
    ...
    inline constexpr friend std::size_t gauss(self& now) {
        std::size_t rk = 0;
    ...
    friend std::ostream& operator<<(std::ostream& os, const self& x) {
        _for(i, 0, x.get_row()) _for(j, 0, x.get_col()) os << x.data(i, j) << (j == x.get_col() - 1 ? '\n' : ' ');
        return os;
    }
#undef _for
#undef _for_row
