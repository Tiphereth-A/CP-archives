...
class mod_integer {
  ...
  public:
    mod_integer() noexcept:
        mod_integer(0) {}
    template <typename Up, _TRAITS(std::is_integral<Up>::value &&std::is_convertible<Up, data_t>::value)>
    mod_integer(const Up &_num) noexcept:
        num(static_cast<data_t>(_num)) { this->safe_mod(); }
...
#define _square_matrix_needed \
  ...
  public:
    ...
    matrix(const std::size_t &_row, const std::size_t &_col, Up &&scalar, const Equal &_equal = Equal()):
        row(_row), col(_col), mat(_row, std::vector<Tp>(_col, scalar)), equ(_equal) {}
    template <typename Up, _TRAITS(_CONVERTIBLE(Up, self &))>
    matrix(Up &&rhs):
        row(std::forward<self>(rhs).get_row()), col(std::forward<self>(rhs).get_col()), mat(std::forward<self>(rhs).mat), equ(std::forward<self>(rhs).equ) {}
    template <typename Up, _TRAITS(_CONVERTIBLE(Up, self))>
    ...
    friend std::ostream &operator<<(std::ostream &os, const self &x) {
        _for(i, 0, x.get_row())
            _for(j, 0, x.get_col()) os << x.data(i, j) << (j == x.get_col() - 1 ? '\n' : ' ');
        return os;
    }
#undef _for
#undef _for_row
