...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  cin >> m >> n;
  vc<int> lst(m * n + 1);
  for_(i, 1, m * n) cin >> lst[i];
  ndvc<2, Node> a(n + 1, m + 1);
  for_(i, 1, n)
    for_(j, 1, m) cin >> a[i][j].id;
  for_(i, 1, n)
    for_(j, 1, m) cin >> a[i][j].cost;
  ndvc<2, int> mac(21, 100001);
  vc<int> step(21), pre(21);
  int ans = 0;
  for_(i, 1, m * n) {
    int now = lst[i], s = 0;
    auto [id, cost] = a[now][++step[now]];
    for (int j = pre[now] + 1;; ++j)
      if ((s = mac[id][j] ? 0 : s + 1) == cost) {
        for_(k, j - cost + 1, j) mac[id][k] = 1;
        chkmax(ans, pre[now] = j);
        break;
      }
  }
  cout << ans;
