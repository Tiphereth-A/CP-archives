...
private:
  ...
  struct TIFA {
    ...
    T sum, val;
    point min, max, coord;
    static CEXP auto get_cmp(u32 k) {
      return [k](const_pointer l, const_pointer r) {
  ...
  CEXP void build(pointer &x, vec<pointer> &h, u32 l, u32 r, u32 k) NE {
    if (l >= r) return;
    u32 mid = l + (r - l) / 2;
    nth_element(
      h.begin() + l, h.begin() + mid, h.begin() + r, TIFA::get_cmp(k));
    x = h[mid], x->sum = x->val;
    flt_(u32, i, 0, K) x->min[i] = x->max[i] = x->coord[i];
    build(x->ch[0], h, l, mid, (k + 1) % K),
      build(x->ch[1], h, mid + 1, r, (k + 1) % K);
    pushup(x);
  }
  CEXP void rebuild(pointer &x) NE {
    ...
    flatten(flatten, x), build(x, h, 0, (u32)h.size(), 0);
  }
  CEXP bool insert_(pointer &x, cT_(point) p, cT_(T) val, u32 k) NE {
    if (!x) {
      x = newnode(p, val);
      return true;
    }
    ...
    else f = insert_(x->ch[p[k] >= x->coord[k]], p, val, (k + 1) % K);
    pushup(x);
    return f;
  }
  ...
  CEXP void query_(const_pointer x, cT_(point) L, cT_(point) R, T &ans) CNE {
    ...
    if (x && check(x->ch[0], L, R)) query_(x->ch[0], L, R, ans);
    if (x && check(x->ch[1], L, R)) query_(x->ch[1], L, R, ans);
  }
...
public:
  ...
  CEXP void insert(cT_(point) coord, cT_(T) val) NE {
    if ((cnt += insert_(root, coord, val, 0)) == B) cnt = 0, rebuild(root);
  }
