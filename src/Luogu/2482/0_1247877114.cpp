...
struct Pig {
    char identity;
    char cards[2005];
    int cards_head = 1, cards_rear = 5;
    int health = 4;
    bool dead;
    bool weapon_wearing;
    int next_player, last_player;
    void debug() {
        std::cerr << "\n\tcards:";
        _rep(i, cards_head, cards_rear) std::cerr << cards[i] << ' ';
        std::cerr << "\n\t\tcards_num:" << cards_rear - cards_head;
        std::cerr << "\n\t\thead:" << cards_head << " rear:" << cards_rear;
        std::cerr << "\n\thealth:" << health;
        std::cerr << "\n\tstate:" << (dead ? "died" : "ailve");
        std::cerr << "\n\tweapon_wearing:" << (weapon_wearing ? "Yes" : "No");
        std::cerr << "\n";
    }
} pigs[15];
namespace Card {
char card_group[2005];
int card_group_head, card_group_rear;
void debug() {
    std::cerr << "\navaliable cards:"
              << card_group_rear - card_group_head << "\n"
              << "\thead:" << card_group_head << " rear:" << card_group_rear << '\n';
    _rep(i, card_group_head, card_group_rear) std::cerr << card_group[i] << ' ';
    std::cerr << '\n';
}
} // namespace Card
using Card::card_group;
using Card::card_group_head;
using Card::card_group_rear;
namespace Relation {
const bool temp[3][3] = {{0, 0, 1},
                         {0, 0, 1},
                         {1, 1, 0}};
...
using Relation::allys;
using Relation::enemys;
using Relation::hostility;
using Relation::semi_enemys;
namespace Identity_list {
int MP_pos;
int ZP_list[15], ZP_cnt;
int FP_list[15], FP_cnt;
void debug() {
    std::cerr << "\nMP_pos:" << MP_pos
              << "\nZP_cnt:" << ZP_cnt << '\n';
...
using Identity_list::FP_cnt;
using Identity_list::FP_list;
using Identity_list::MP_pos;
using Identity_list::ZP_cnt;
using Identity_list::ZP_list;
namespace main_action {
bool game_fin;
#define _main_action_of_use_f(_now, _from)
{
    int pos = -1;
    if (~(pos = find_card((_now), 'K')))
        clr_card((_now), pos);
    else
        break;
}
...
void clr_card(int now_player, int pos) {
    ...
    if (pos == now->cards_head)
        while (now->cards_head < now->cards_rear && now->cards[now->cards_head] == '\0')
            ++now->cards_head;
}
...
void print_result() {
    ...
    _fp(i, 1, n) {
        if (pigs[i].dead)
            ...
            _rep(j, pigs[i].cards_head, pigs[i].cards_rear) {
                if (pigs[i].cards[j])
                    cout << pigs[i].cards[j] << ' ';
            }
        cout << '\n';
...
bool use_j(int now_player, int from_player, int target_player, bool state = true) {
        ...
        if (!pigs[next_player].dead) {
            if (hostility(next_player, target_player) ^ state) {
                ...
                if (~pos) {
                    ...
                    if (pigs[next_player].identity == 'F')
                        enemys[next_player] = 1;
                    else
                        allys[next_player] = 1;
...
bool use_f(int now_player, int from_player) {
    ...
    if ((allys[now_player] || enemys[now_player]) && use_j(from_player, from_player, now_player))
        return 0;
    ...
    while (1) {
        if (ima_no_pureiyaa) {
            _main_action_of_use_f(from_player, now_player);
        } else {
            _main_action_of_use_f(now_player, from_player);
        }
        if (from->dead)
            return 1;
        ima_no_pureiyaa ^= 1;
...
void use_nw(int from_player, char card) {
    ...
    for (int i = pigs[from_player].next_player; i != from_player; i = pigs[i].next_player) {
        Pig* now = &pigs[i];
        int pos = -1;
        if ((allys[i] || enemys[i]) && use_j(from_player, from_player, i))
            continue;
...
void action(int now_player) {
    Pig* now = &pigs[now_player];
    int head_now = now->cards_head;
    int target_pos = now->next_player;
    bool used_K = 0;
        ...
        switch (now->cards[head_now]) {
            ...
            case 'K':
                if (now->weapon_wearing || !used_K) {
                    switch (now->identity) {
                        case 'M':
                            if (enemys[target_pos] || (!allys[target_pos] && semi_enemys[target_pos])) {
                                clr_card(now_player, head_now);
                ...
                switch (now->identity) {
                    case 'M':
                        for (int i = target_pos; i != now_player; i = pigs[i].next_player) {
                            if ((enemys[i] || (!allys[i] && semi_enemys[i])) && !pigs[i].dead) {
                                clr_card(now_player, head_now);
                                if (use_f(i, now_player))
                                    return;
                                head_now = now->cards_head - 1;
                                break;
                            }
