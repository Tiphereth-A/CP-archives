...
const bool temp[3][3] = {{0, 0, 1},
                         {0, 0, 1},
                         {1, 1, 0}};
int n, m, card_group_head, card_group_rear, MP_pos, ZP_list[15], ZP_cnt, FP_list[15], FP_cnt;
...
void print_result() {
    ...
    _fp(i, 1, n) {
        if (pigs[i].dead)
            ...
            _rep(j, pigs[i].cards_head, pigs[i].cards_rear) if (pigs[i].cards[j]) cout << pigs[i].cards[j] << ' ';
        cout << '\n';
    }
}
...
bool use_f(int now_player, int from_player) {
    ...
    while (1) {
        if (ima_no_pureiyaa) _main_action_of_use_f(from_player, now_player) else _main_action_of_use_f(now_player, from_player) if (from->dead) return 1;
        ima_no_pureiyaa ^= 1;
    }
    if (ima_no_pureiyaa) {
        --from->health;
