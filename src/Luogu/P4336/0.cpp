...
namespace std {
uint32_t abs(mint x) { return x.val(); }
}  // namespace std
...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  for_(i, 2, n, x) {
    cin >> x;
    e.emplace_back(x);
    for (auto &j : e.back()) cin >> j;
  }
  mint ans = 0;
  int S = (1 << (n - 1)) - 1;
  for_(state, 1, S) {
    matrix<mint> mat(n, n, [](mint const &x) { return x.val() == 0; }, 0);
    for_(i, 0, n - 2) {
    ...
    for_(i, 0, n - 1) mat(i, i) = -mat.row_varray(i).sum();
    mat *= -1;
    ans += mint((n - __builtin_popcount(state)) % 2 ? 1 : -1) *
           mat.submatrix(1, n, 1, n).det();
  }
  cout << ans;
}
int main() {
