...
#undef SCALA_FUNC
    ...
    inline size_t is_containing_cvh(const point_t &p) const {
        ...
        return is_zero(res) && !is_positive(dot(p, *it, *next_it)) ? self::inside_or_onborder : self::outside;
    }
#define SELF_FUNC(name, func_body)      \
...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    read_var_(int, m);
    read_var_(int, q);
    Polynomial2 polya(n), polyb(m);
    cin >> polya;
    for_(i, 0, m - 1, x, y) {
        cin >> x >> y;
        polyb[i] = {-x, -y};
    }
    polya.do_convex_hull().do_minkowski_sum_cvh(polyb.do_convex_hull());
    for_(i, 1, q, x, y) {
        cin >> x >> y;
        cout << !!polya.is_containing_cvh({x, y}) << '\n';
    }
}
int main() {
