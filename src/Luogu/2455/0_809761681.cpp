...
int main() {
    ...
    _for(i, 1, n, m) {
        m = now;
        _for(j, now + 1, n) if (abs(a[j][i]) > abs(a[m][i])) m = j;
        if (_double_equ(a[m][i], 0)) continue;
        if (m != now) _for(j, 1, n + 1) swap(a[m][j], a[now][j]);
        _for(j, 1, n) {
            if (j == now) continue;
    ...
    if (now != n + 1) {
        _for(i, now, n) if (!_double_equ(a[i][n + 1], 0)) {
            cout << "-1";
            return 0;
        }
        cout << "0";
    ...
    _for(i, 1, n) if (_double_equ(a[i][n + 1] /= a[i][i], 0)) a[i][n + 1] = 0;
