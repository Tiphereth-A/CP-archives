...
struct ANT {
    ...
    int age;
    int level;
    ANT(int X_now = 0, int Y_now = 0, int X_pre = 0, int Y_pre = 0, int Blood = 0, bool Taking_cake = false, int Age = 0, int Level = 0) {
        x_now = X_now;
        y_now = Y_now;
        x_pre = X_pre;
        y_pre = Y_pre;
...
struct TURRET {
    ...
    void attack() {
        if (!cake_disappeared || get_dis_sqr(x, y, ant[target_all].x_now, ant[target_all].y_now) > r_sqr) {
            ...
            if (dis_sqr_min > r_sqr)
                return;
            ant[target_antID].blood_now -= d;
            return;
        } else {
            i64 A = 1ll * y - ant[target_all].y_now, B = 1ll * ant[target_all].x_now - x;
            i64 C = -1ll * (x * A + y * B);
            i64 x1 = ant[target_all].x_now - x, y1 = ant[target_all].y_now - y;
            i64 dis_tmp = get_dis_sqr(x, y, ant[target_all].x_now, ant[target_all].y_now);
            _T_ANT(i) {
                if (ant[i].taking_cake) {
                    ant[i].blood_now -= d;
                    continue;
                }
                if (get_dis_sqr(x, y, ant[i].x_now, ant[i].y_now) > dis_tmp)
                    continue;
...
void ant_clear(int antID) {
    for (int i = antID + 1; i <= ant_alive; ++i) {
        ant[i - 1] = ant[i];
    }
    --ant_alive;
}
