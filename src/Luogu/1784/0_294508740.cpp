...
inline int get_zone(int x, int y) {
...
void dfs(int x = 1, int y = 1) {
    if (g[x][y]) {
        ...
        bool *tmp_ln = &ln[x][1], *tmp_col = &col[y][1], *tmp_zone = &zone[get_zone(x, y)][1];
        for (int i = 1, *tmp_g = &g[x][y]; i <= 9; ++i, ++tmp_ln, ++tmp_col, ++tmp_zone) {
            if (*tmp_ln || *tmp_col || *tmp_zone)
                continue;
