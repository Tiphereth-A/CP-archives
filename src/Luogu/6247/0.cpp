...
data_t min_dist(const cont_t<Point2> &poly, ptrdiff_t l, ptrdiff_t r) {
    data_t ret = DATA_MAX;
    if (r - l <= 5) {
        for (ptrdiff_t i = l; i < r; ++i)
            for (ptrdiff_t j = l; j < i; ++j) ret = std::min(ret, dist_PP(poly[i], poly[j]));
        return ret;
    }
...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    Polynomial2 poly{vp};
    poly.resort();
    cout << min_dist(poly.vertice, 0, n) << ' ' << poly.do_convex_hull().convex_diameter();
}
int main() {
