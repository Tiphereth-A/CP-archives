...
struct Node {
    i64 x, y;
    i64 cnt;
    Node(i64 x = 0, i64 y = 0, i64 cnt = 0):
        x(x), y(y), cnt(cnt) {}
};
inline auto solve([[maybe_unused]] int t_) -> void {
    auto comp_x = [](const Node &lhs, const Node &rhs) { return lhs.x == rhs.x ? lhs.y < rhs.y : lhs.x < rhs.x; };
    auto comp_y = [](const Node &lhs, const Node &rhs) { return lhs.y == rhs.y ? lhs.x < rhs.x : lhs.y < rhs.y; };
    read_var_(int, n);
    i64 x, y;
    i64 t;
    vc<Node> vec, vec2;
    ...
    auto &&[_, xx, yy] = max({make_tuple(calc(x, y), x, y), make_tuple(calc(x + 1, y), x + 1, y), make_tuple(calc(x, y + 1), x, y + 1), make_tuple(calc(x + 1, y + 1), x + 1, y + 1)});
    debug(x, y, xx, yy);
    cout << xx << ' ' << yy;
}
int main() {
