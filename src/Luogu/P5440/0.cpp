...
const auto STATIC__ = []() {
  for_(i, 1, 12)
    for (int j = 1; p[j] <= d[i]; ++j)
      if (is_prime(i * 100 + p[j])) a.push_back(i * 100 + p[j]);
  for_step_(i, 4, 9999, 4)
    if ((i % 100 || !(i % 400)) && is_prime(i * 10000 + 229))
      ans.push_back(i * 10000 + 229);
  for_(i, 1, 9999)
    for (auto &&j : a)
      if (is_prime(i * 10000 + j)) ans.push_back(i * 10000 + j);
  return 0;
}();
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  for (auto now : ans) {
    int flag = 1;
    for (int j = 7; flag && ~j; --j, now /= 10)
      if (s[j] != '-' && s[j] - '0' != now % 10) flag = 0;
    cnt += flag;
  }
  cout << cnt << '\n';
}
