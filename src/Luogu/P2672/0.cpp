...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  sort(all_(a), [](auto &&l, auto &&r) { return l.second > r.second; });
  vc<i64> ans(n);
  ans[0] = a[0].second;
  for_(i, 1, n - 1) ans[i] = ans[i - 1] + a[i].second;
  vc<i64> ss(n);
  ss[0] = 2 * a[0].first;
  for_(i, 1, n - 1) ss[i] = max(ss[i - 1], 2 * a[i].first);
  vc<i64> ssa(n);
  ssa.back() = 2 * a.back().first + a.back().second;
  rfor_(i, n - 2, 0) ssa[i] = max(ssa[i + 1], 2 * a[i].first + a[i].second);
  for_(i, 0, n - 1)
    cout << max(ans[i] + ss[i], (i ? ans[i - 1] : 0) + ssa[i]) << '\n';
}
int main() {
