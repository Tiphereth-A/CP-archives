...
public:
  explicit DLX(size_t col = 0): node(), cnt_col() { reset(col); }
  explicit DLX(std::vector<std::vector<bool>> const &grid) { reset_grid(grid); }
  friend std::ostream &operator<<(std::ostream &os, DLX const &dlx) {
    os << dlx.node[0].l << ' ' << dlx.cnt_col.size() << ' ' << dlx.node.size()
       << '\n';
  ...
  void reset(size_t col) {
    ...
    node.emplace_back(col, 1, 0, 0, 0, 0);
