...
public:
  friend std::ostream &operator<<(std::ostream &os, DLX const &dlx) {
    os << dlx.width << ' ' << dlx.height << ' ' << dlx.cnt_node << '\n';
    for (size_t i = 1; i <= dlx.width; ++i)
      os << dlx.cnt_col[i] << " \n"[i == dlx.width];
    for (size_t i = 0; i <= dlx.cnt_node; ++i)
      os << i << ": " << dlx.node[i] << '\n';
    return os;
  }

public:
  DLX(): node(), cnt_col(), width(0), height(0), cnt_node(0) {}
  DLX(size_t width, size_t height): DLX() { reset(width, height); }
  DLX(size_t width, size_t height, std::vector<std::vector<bool>> const &grid)
    : DLX() {
  ...
  void reset(size_t width_, size_t height_) {
    node.resize((width_ + 1) * height_ + 1);
    cnt_col.resize(height_ + 1);
    width = cnt_node = width_;
    height = height_;
    for (size_t i = 1; i <= width; ++i) node[i] = {i - 1, i + 1, i, i, 0, i};
    node[r_(width) = 0] = {width, 1, 0, 0, 0, 0};
  }
  void reset_grid(size_t width_,
                  size_t height_,
                  std::vector<std::vector<bool>> const &grid) {
    reset(width_, height_);
    std::vector<size_t> _;
    for (size_t ln_ = 0; ln_ < height_; ++ln_) {
      _.clear();
      for (size_t col_ = 0; col_ < width_; ++col_)
        if (grid[ln_][col_]) _.push_back(col_ + 1);
      if (!_.empty()) insert_row(ln_ + 1, _);
    }
  }
  void insert_row(size_t ln_, const std::vector<size_t> &cols_) {
    ...
    for (size_t i = 0; i < cols_.size(); ++i) {
      node[cnt_node + i + 1] = {
        cnt_node + i, cnt_node + i + 2, u_(cols_[i]), cols_[i], ln_, cols_[i]};
      d_(u_(cols_[i])) = cnt_node + i + 1;
      u_(cols_[i]) = cnt_node + i + 1;
      ++cnt_col[cols_[i]];
      if (d_(cols_[i]) == cols_[i]) d_(cols_[i]) = cnt_node + i + 1;
    }
    l_(cnt_node + 1) = cnt_node + cols_.size();
    r_(cnt_node + cols_.size()) = cnt_node + 1;
    cnt_node += cols_.size();
  }
