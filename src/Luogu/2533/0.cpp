...
struct Point_2D {
    ...
    Point_2D operator+(const Point_2D &rhs) const { return Point_2D(x + rhs.x, y + rhs.y); }
    Point_2D operator-(const Point_2D &rhs) const { return Point_2D(x - rhs.x, y - rhs.y); }
...
double sqr_add_add(const Point_2D &a, const Point_2D &b) { return ((a.x * a.x + b.x * b.x) + (a.y * a.y + b.y * b.y)) / 2; }
double sqr_add_minus(const Point_2D &a, const Point_2D &b) { return ((a.x * a.x + b.x * b.x) - (a.y * a.y + b.y * b.y)) / 2; }
double sqr_minus_add(const Point_2D &a, const Point_2D &b) { return ((a.x * a.x - b.x * b.x) + (a.y * a.y - b.y * b.y)) / 2; }
double sqr_minus_minus(const Point_2D &a, const Point_2D &b) { return ((a.x * a.x - b.x * b.x) - (a.y * a.y - b.y * b.y)) / 2; }
Point_2D mid_point(const Point_2D &a, const Point_2D &b) { return (a + b) / 2; }
double inner_prod(const Point_2D &a, const Point_2D &b) { return a.x * b.x + a.y * b.y; }
double outer_prod(const Point_2D &a, const Point_2D &b) { return a.x * b.y - a.y * b.x; }
double distance(const Point_2D &a, const Point_2D &b) { return sqrt(sqr_add(a - b)); }
}  // namespace _Point_2D
...
struct Circle {
    Point_2D c;
    double d;
    Circle(const Point_2D &c = Point_2D(), double d = 0):
        c(c), d(d) {}
...
Circle get_circle(const Point_2D &a, const Point_2D &b, const Point_2D &c) {
    ...
    Circle ret;
    ret.c = Point_2D((_b.y * _c - _a.y * _d) / outer_prod(_a, _b), (_a.x * _d - _b.x * _c) / outer_prod(_a, _b));
        ...
        if (ret.out_circle(a[i])) {
            ...
            for (int j = 1; j < i; ++j) {
                if (ret.out_circle(a[j])) {
                    ...
                    for (int k = 1; k < j; ++k)
                        if (ret.out_circle(a[k])) {
                            ret = get_circle(a[i], a[j], a[k]);
                        }
                }
            }
        }
