...
auto solve([[maybe_unused]] int t_) -> void {
  ...
  while (i < cvh.vs.size() && j < n) {
    if (i && is_equal(cvh[i].x, cvh[i - 1].x) &&
        is_greater(cvh[i].y, cvh[i - 1].y))
      run_continue_(++i);
    if (cvh[i].x > x[j]) {
      Point2 temp =
        ins_LL({cvh[i], cvh[i - 1]}, {{x[j], y[j]}, {x[j], y[j] + 1}});
      chkmin(ans, abs(temp.y - y[j++]));
    } else {
      Point2 temp = ins_LL({{x[j - 1], y[j - 1]}, {x[j], y[j]}},
                           {cvh[i], {cvh[i].x, cvh[i].y + 1}});
