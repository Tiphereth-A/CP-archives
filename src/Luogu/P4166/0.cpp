...
struct Triangle {
  ...
  constexpr Point2 center_H() const {
    return ins_LL({A, A + (B - C).do_rotate90()},
                  {B, B + (C - A).do_rotate90()});
  }
...
struct Circle2 {
  ...
  constexpr data_t crown_area(data_t angle = PI * 2) const {
    return (angle - std::sin(angle)) / 2 * r * r;
  }
};
...
data_t sarea_CT(const Circle2 &c, const Point2 &p1, const Point2 &p2) {
  ...
  if (b1 && b2) {
    if (!is_positive(dot(is[0], p1, p2)) && !is_positive(dot(is[1], p1, p2)))
      return c.area(ang_PP(p1 - c.o, is[0] - c.o) +
                    ang_PP(is[1] - c.o, p2 - c.o)) +
             cross_mul(is[0], is[1]) / 2;
    return c.area(ang_PP(p1 - c.o, p2 - c.o));
  }
  if (b1)
    return cross_mul(is[0], p2) / 2 + c.area(ang_PP(p1 - c.o, is[0] - c.o));
  if (b2)
    return cross_mul(p1, is[1]) / 2 + c.area(ang_PP(is[1] - c.o, p2 - c.o));
...
auto solve([[maybe_unused]] int t_) -> void {
  ...
  if (sz == 3) {
    ...
    foreach_cref_(p, poly.vs) {
      ...
      chkmax(ans,
             raw - min({abs(cross(p, cvh[0], cvh[1])),
                        abs(cross(p, cvh[1], cvh[2])),
                        abs(cross(p, cvh[2], cvh[0]))}));
    }
    run_return_void_(cout << ans / 2);
  }
  auto check = [](const Point2 &a,
                  const Point2 &b,
                  const Point2 &c,
                  const Point2 &d) -> bool {
    return abs(cross(a, b, c)) < abs(cross(a, b, d));
  };
  for_(i, 0, sz - 1, j, k) {
    j = (i + 1) % sz;
    k = (i + 2) % sz;
    for (size_t i2 = k; i2 != i; i2 = cvh.next(i2)) {
      while (j != i2 && check(cvh[i], cvh[i2], cvh[j], cvh[cvh.next(j)]))
        j = cvh.next(j);
      while (k != i && check(cvh[i], cvh[i2], cvh[k], cvh[cvh.next(k)]))
        k = cvh.next(k);
      chkmax(ans,
             abs(cross(cvh[i], cvh[i2], cvh[j])) +
               abs(cross(cvh[i], cvh[i2], cvh[k])));
    }
  }
  cout << ans / 2;
}
