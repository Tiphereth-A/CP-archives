...
struct Triangle {
    ...
    friend std::istream &operator>>(std::istream &is, self &rhs) { return is >> rhs.A >> rhs.B >> rhs.C; }
    friend std::ostream &operator<<(std::ostream &os, const self &rhs) { return os << rhs.A << ' ' << rhs.B << ' ' << rhs.C; }
...
struct Circle2 {
    ...
    constexpr Circle2(const data_t &c_x, const data_t &c_y, data_t r_):
        o(c_x, c_y), r(r_) {}
    friend std::istream &operator>>(std::istream &is, self &rhs) { return is >> rhs.o >> rhs.r; }
    friend std::ostream &operator<<(std::ostream &os, const self &rhs) { return os << rhs.o << ' ' << rhs.r; }
...
#undef SCALA_FUNC
    ...
    inline point_t mass_point() const {
        point_t ret{};
        data_t area{};
...
struct ConvexHull2: public Polynomial2 {
    ...
    friend inline bool is_ins_cvh(const self &cvhl, const self &cvhr) {
    ...
    friend inline base coverage_rect_with_min_circum(self &lhs) {
        size_t sz = lhs.vs.size();
        if (sz == 0) return lhs;
        if (sz == 1) return base{cont_t<point_t>{lhs[0], lhs[0], lhs[0], lhs[0]}};
        if (sz == 2) return base{cont_t<point_t>{lhs[0], lhs[0], lhs[1], lhs[1]}};
    ...
    inline self &do_minkowski_sum_nonstrict(const self &rhs) {
    ...
    friend inline self minkowski_sum_nonstrict(self poly, const self &rhs) { return poly.do_minkowski_sum_nonstrict(rhs); }
    inline self &do_minkowski_sum(const self &rhs) { return do_minkowski_sum_nonstrict(rhs).init(); }
    friend inline self minkowski_sum(self poly, const self &rhs) { return poly.do_minkowski_sum(rhs); }
