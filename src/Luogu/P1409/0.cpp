...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  int n, m;
  cin >> n >> m;
  ndvc<2, double> dp(n + 1, n + 1);
  dp[1][1] = 1;
  auto f = [&dp](auto &&f, int n, int m) -> double {
    if (dp[n][m]) return dp[n][m];
    if (m == 1) return dp[n][m] = f(f, n, n) / 2 + 1. / 6;
    if (n == m) {
      double ans = pow(.5, n);
      for_(k, 1, n - 1) ans += f(f, n - 1, k) * pow(.5, n - k - 1);
      return dp[n][n] = ans / 3 / (1 - pow(.5, n));
    }
    return dp[n][m] = f(f, n, m - 1) / 2 + f(f, n - 1, m - 1) / 3;
  };
  cout << fixed << setprecision(9) << f(f, n, m);
}
int main() {
