...
int64_t ans[N][N];
inline bool init(int vertex_num, int edge_num, const tuple<int, int, int> *edges) {
    n = vertex_num;
    m = edge_num;
...
inline auto solve() -> void {
    ...
    _for(i, 1, n) {
        i64 ans = 0;
        _for(j, 1, n) ans += j * (Johnson_shortest_path::is_disconnected(i, j) ? (i64)(1e9) : Johnson_shortest_path::distance(i, j));
        cout << ans << '\n';
    }
}
int main() {
