...
const u32 MOD = 19940417;
const u32 INV3 = 6646806;
using namespace std;
constexpr i64 sum1(i64 n) { return n * (n + 1) / 2 % MOD; }
constexpr i64 sum2(i64 n) {
  return n * (n + 1) / 2 % MOD * (2 * n + 1) % MOD * INV3 % MOD;
}
constexpr i64 f1(i64 n) {
  i64 ans = 0;
  for (i64 l = 1, r = 1; l <= n; l = r + 1) {
    r = n / (n / l);
    ans += MOD - (sum1(r) + MOD - sum1(l - 1)) * (n / l) % MOD;
  }
  return (n * n % MOD + ans % MOD) % MOD;
}
constexpr i64 f2(i64 n, i64 m) {
  i64 ans = 0;
  for (i64 l = 1, r = 1; l <= n; l = r + 1) {
    r = min(n / (n / l), m / (m / l));
    ans += MOD - (sum1(r) + MOD - sum1(l - 1)) *
                   ((n / l) * m % MOD + (m / l) * n % MOD) % MOD;
    ans += (sum2(r) + MOD - sum2(l - 1)) * (n / l) % MOD * (m / l) % MOD;
  }
  return (n * n % MOD * m % MOD + ans % MOD) % MOD;
}
auto solve([[maybe_unused]] int t_ = 0) -> void {
  i64 n, m;
  cin >> n >> m;
  if (n > m) swap(n, m);
  cout << (f1(n) * f1(m) % MOD + MOD - f2(n, m)) % MOD;
}
int main() {
