...
class BIT {
  ...
  public:
    ...
    Tp query(ptrdiff_t pos) const {
        Tp ret = 0;
        for (ptrdiff_t i = clamp(pos, ptrdiff_t(0), ptrdiff_t(N) - 1); i; i -= lowbit(i)) ret += tree[i];
        return ret;
    }
};
BIT<150'005, i64> tr;
i64 main(i64 n, i64 d, i64 m) {
    ...
    i64 ans = 0;
    for_(i, 0, n - 1, j = 0) {
        while (j < i && vec[i].first - vec[j].first > d) tr.modify(vec[j++].second + m, -1);
        ans += tr.query(vec[i].second + d + m) - tr.query(vec[i].second - d - 1 + m);
...
class BIT3 {
  ...
  public:
    ...
    Tp query(ptrdiff_t x, ptrdiff_t y, ptrdiff_t z) const {
        Tp ret = 0;
        for (ptrdiff_t i = clamp(x, ptrdiff_t(0), ptrdiff_t(N) - 1); i; i -= lowbit(i))
            for (ptrdiff_t j = clamp(y, ptrdiff_t(0), ptrdiff_t(N) - 1); j; j -= lowbit(j))
                for (ptrdiff_t k = clamp(z, ptrdiff_t(0), ptrdiff_t(N) - 1); k; k -= lowbit(k)) ret += tree[i][j][k];
        return ret;
    }
};
BIT3<259, i64> tr;
i64 query(i64 l1, i64 r1, i64 l2, i64 r2, i64 l3, i64 r3) { return tr.query(r1, r2, r3) - tr.query(l1, r2, r3) - tr.query(r1, l2, r3) - tr.query(r1, r2, l3) + tr.query(l1, l2, r3) + tr.query(l1, r2, l3) + tr.query(r1, l2, l3) - tr.query(l1, l2, l3); }
i64 main(i64 n, i64 d, i64 m) {
    tr.clear();
    vc<pi4i64> vec;
    vec.reserve(n);
    ...
    for_(i, 0, n - 1, j = 0) {
        ...
        debug(j + 1);
        debug(get<1>(vec[i]) - d - 1 + m, get<1>(vec[i]) + d + m, get<2>(vec[i]) - d - 1 + m, get<2>(vec[i]) + d + m, get<3>(vec[i]) - d - 1 + m, get<3>(vec[i]) + d + m);
        ans += query(get<1>(vec[i]) - d - 1 + m, get<1>(vec[i]) + d + m, get<2>(vec[i]) - d - 1 + m, get<2>(vec[i]) + d + m, get<3>(vec[i]) - d - 1 + m, get<3>(vec[i]) + d + m);
        debug_withname_(ans);
