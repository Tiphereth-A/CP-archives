...
constexpr data_t PI = std::acos(data_t{-1});
...
#define SCALA_FUNC(name, func_body)        \
    ...
    SCALA_FUNC(arg_2pi, { data_t tmp_= arg(); return is_negative(tmp_) ? tmp_ + 2 * PI : tmp_; })
...
struct Circle2 {
    ...
    constexpr data_t area(data_t angle = PI * 2) const { return angle / 2 * r * r; }
    constexpr data_t crown_area(data_t angle = PI * 2) const { return (angle - std::sin(angle)) / 2 * r * r; }
};
...
inline data_t sarea_CT(const Circle2 &c, const Point2 &p1, const Point2 &p2) {
    ...
    if (b1 && b2) {
        if (!is_positive(dot(is[0], p1, p2)) && !is_positive(dot(is[1], p1, p2))) return c.area(ang_PP(p1 - c.o, is[0] - c.o) + ang_PP(is[1] - c.o, p2 - c.o)) + cross_mul(is[0], is[1]) / 2;
        return c.area(ang_PP(p1 - c.o, p2 - c.o));
    }
    if (b1) return cross_mul(is[0], p2) / 2 + c.area(ang_PP(p1 - c.o, is[0] - c.o));
    if (b2) return cross_mul(p1, is[1]) / 2 + c.area(ang_PP(is[1] - c.o, p2 - c.o));
...
struct Polynomial2 {
    ...
    inline cont_t<Point2>::iterator prev(cont_t<Point2>::iterator it) {
        if (it == vs.begin()) it = vs.end();
        return --it;
    }
    inline cont_t<Point2>::const_iterator prev(cont_t<Point2>::const_iterator it) const {
        if (it == vs.begin()) it = vs.end();
        return --it;
    }
    inline cont_t<Point2>::iterator next(cont_t<Point2>::iterator it) {
        if (++it == vs.end()) it = vs.begin();
        return it;
    }
    inline cont_t<Point2>::const_iterator next(cont_t<Point2>::const_iterator it) const {
        if (++it == vs.end()) it = vs.begin();
        return it;
    }
...
struct ConvexHull2: public Polynomial2 {
    ...
    inline ConvexHull2 &do_convex_cut(const Line2 &l) {
        size_t sz = vs.size();
        cont_t<Point2> cvc;
        for (size_t i = 0; i < sz; ++i) {
    ...
    friend inline ConvexHull2 convex_cut(ConvexHull2 poly, const Line2 &l) { return poly.do_convex_cut(l); }
};
...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    Polynomial2 poly(n);
    cin >> poly;
    ConvexHull2 cvh{poly.vs};
    size_t sz = cvh.vs.size();
    if (sz < 3) run_return_void_(cout << "0.000\n");
    data_t ans = 0;
    int flag = 0;
    if (sz == 3) {
        data_t raw = abs(cross(cvh[0], cvh[1], cvh[2]));
        foreach_cref_(p, poly.vs) {
            if (!(flag & 1) && p == cvh[0]) run_continue_(flag |= 1);
            if (!(flag & 2) && p == cvh[1]) run_continue_(flag |= 2);
            if (!(flag & 4) && p == cvh[2]) run_continue_(flag |= 4);
            chkmax(ans, raw - min({abs(cross(p, cvh[0], cvh[1])), abs(cross(p, cvh[1], cvh[2])), abs(cross(p, cvh[2], cvh[0]))}));
        }
        run_return_void_(cout << ans / 2);
    }
    auto check = [](const Point2 &a, const Point2 &b, const Point2 &c, const Point2 &d) -> bool { return abs(cross(a, b, c)) < abs(cross(a, b, d)); };
    for_(i, 0, sz - 1, j, k) {
        j = (i + 1) % sz;
        k = (i + 2) % sz;
        for (size_t i2 = k; i2 != i; i2 = cvh.next(i2)) {
            while (j != i2 && check(cvh[i], cvh[i2], cvh[j], cvh[cvh.next(j)])) j = cvh.next(j);
            while (k != i && check(cvh[i], cvh[i2], cvh[k], cvh[cvh.next(k)])) k = cvh.next(k);
            chkmax(ans, abs(cross(cvh[i], cvh[i2], cvh[j])) + abs(cross(cvh[i], cvh[i2], cvh[k])));
        }
    }
    cout << ans / 2;
}
