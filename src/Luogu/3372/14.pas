...
procedure pushup(p:int64); begin
...
procedure pushdown(p,ln,rn:int64); begin
    if(tag[p]<>0) then begin
...
begin
    ...
    if(ul<=mid) then update(p shl 1,l,mid,ul,ur,k);
    if(mid<ur) then update((p shl 1) or 1,mid+1,r,ul,ur,k);
    pushup(p);
end;
function query(p,l,r,ul,ur:int64):int64;
var ans,mid:int64;
begin
    if((ul<=l) and (r<=ur)) then exit(tree[p]);
    ans:=0;
    mid:=(l+r) shr 1;
    pushdown(p,mid-l+1,r-mid);
    if(ul<=mid) then ans:=ans+query(p shl 1,l,mid,ul,ur);
    if(mid<ur) then ans:=ans+query((p shl 1) or 1,mid+1,r,ul,ur);
    exit(ans);
end;
begin
    read(n,m);
    ...
    for i:=1 to m do begin
        ...
        if(op=1) then begin
            read(k);
            update(1,1,n,x,y,k);
        end else writeln(query(1,1,n,x,y));
    end;
end.