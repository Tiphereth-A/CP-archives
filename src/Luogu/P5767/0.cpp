...
class FloydSolver {
  private:
    size_t n;
    Tp spath[N][N];
    const static Tp INF = std::numeric_limits<Tp>::max() / 2;

  public:
    using self = FloydSolver<N, Tp>;
    FloydSolver() {}
    inline self &init(size_t _n = N - 1) {
        n = _n;
        for (size_t i = 0; i <= n; ++i) std::fill(spath[i], spath[i] + n + 1, INF);
        for (size_t i = 0; i <= n; ++i) spath[i][i] = 0;
        return *this;
    }
    inline self &addedge(size_t from, size_t to, Tp w = 1) {
        spath[from][to] = w;
        return *this;
    }
    inline self &floyd() {
    ...
    inline const Tp &query(size_t from, size_t to) const { return spath[from][to]; }
    inline const bool reachable(size_t from, size_t to) const { return query(from, to) != INF; }
};
