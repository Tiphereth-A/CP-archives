...
void update(int L, int R, i64 k, int l, int r, int p, int f) {
    if (L <= l && r <= R) {
        if (f) {
    ...
    if (L <= m)
        update(L, R, k, l, m, ls, f);
    if (R > m)
        update(L, R, k, m + 1, r, rs, f);
    PushUp(p);
}
i64 query(int L, int R, int l, int r, int p) {
    if (L <= l && r <= R)
        return sum[p];
    Mid;
    PushDown(p, m - l + 1, r - m);
    i64 ans = 0;
    if (L <= m)
        ans = (ans + query(L, R, l, m, ls)) % mod;
    if (R > m)
        ans = (ans + query(L, R, m + 1, r, rs)) % mod;
    return ans;
}
int main() {
    int n, m;
