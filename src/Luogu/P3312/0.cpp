...
using namespace std;
namespace primes {
const uint32_t N = 2e6 + 5;
const uint32_t P = 2e5 + 5;
int32_t min_pfactor[N];
int32_t prime[P], cnt;
int64_t pw[N], sigma[N];
int64_t mu[N];
void init_prime(const uint32_t &n = N - 1) {
  sigma[1] = mu[1] = 1;
  for (uint32_t i = 2; i <= n; ++i) {
    if (!min_pfactor[i]) {
      min_pfactor[i] = prime[++cnt] = i;
      pw[i] = (int64_t)i * i;
      sigma[i] = i + 1;
      mu[i] = -1;
    }
    for (uint32_t j = 1; j <= cnt && i * prime[j] <= n; ++j) {
      uint32_t x = i * prime[j];
      min_pfactor[x] = prime[j];
      if (i % prime[j]) {
        pw[x] = (int64_t)prime[j] * prime[j];
        sigma[x] = sigma[i] * (prime[j] + 1);
        mu[x] = -mu[i];
      } else {
        sigma[x] = sigma[i] * ((pw[x] = pw[i] * prime[j]) - 1) / (pw[i] - 1);
      }
    }
  }
}
}  // namespace primes
using primes::init_prime, primes::prime, primes::cnt;
using primes::min_pfactor;
using primes::mu;
using primes::sigma;
...
struct Node {
  int n, m, a, id;
  Node(int n, int m, int a, int id): n(n), m(m), a(a), id(id) {}
  bool operator<(Node const &rhs) const { return a < rhs.a; }
};
auto solve([[maybe_unused]] int t_ = 0) -> void {
  init_prime();
  vc<pii64> d;
  for_(i, 1, primes::N - 1) d.emplace_back(sigma[i], i);
  sort(all_(d));
  read_var_(int, q);
  vc<Node> b;
  for_(i, 0, q - 1, n, m, a) {
    cin >> n >> m >> a;
    b.emplace_back(n, m, a, i);
  }
  sort(all_(b));
  BIT<i64> g(N);
  auto calc = [&](i64 n, i64 m) -> i64 {
    if (n > m) swap(n, m);
    i64 ans = 0;
    for (i64 l = 1, r; l <= n; l = r + 1) {
      r = min(n / (n / l), m / (m / l));
      ans += (n / l) * (m / l) * (g.query(r) - g.query(l - 1));
    }
    return ans;
  };
  vc<i64> ans(q);
  int pre = 0;
  for (auto [n, m, a, id] : b) {
    while (pre < d.size() && d[pre].first <= a) {
      auto [now_d, now_x] = d[pre++];
      for (i64 k = 1; k * now_x < N; ++k) g.modify(k * now_x, now_d * mu[k]);
    }
    ans[id] = calc(n, m);
  }
  for (auto &&i : ans) cout << (i & 0x7fffffff) << '\n';
}
int main() {
