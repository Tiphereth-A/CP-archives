...
public:
  ...
  RombergSolver(Func &&F): f(F) {}
  double operator()(double a, double b, double eps) const {
    double h = b - a;
    double T1 = T1 = (f(a) + f(b)) * h / 2, T2 = 0, S1 = 0, S2 = 0, C1 = 0,
           C2 = 0, R1 = 0, R2 = 0;
    int k = 1;
    while (1) {
      double S = ctqf(a, b, h);
...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  double a, b, c, d, l, r;
  cin >> a >> b >> c >> d >> l >> r;
  auto f = [a, b, c, d](double x) { return (c * x + d) / (a * x + b); };
  cout << fixed << setprecision(6) << RombergSolver(f)(l, r, EPS);
}
int main() {
