...
void debug() {
  std::cerr << "\navaliable cards:" << card_group_rear - card_group_head << "\n"
...
void print_result() {
  ...
  _for(i, 1, n) {
    ...
    else _rep(j, pigs[i].cards_head, pigs[i].cards_rear) {
        if (pigs[i].cards[j]) cout << pigs[i].cards[j] << ' ';
      }
    cout << '\n';
  }
}
