...
#define _mid(l, r) (((l) + (r)) >> 1)
    ...
    Node data[N << 2];
    data_type tag[N << 2];
    inline void _push_up(int p) { (data[p] = data[_lch(p)] + data[_rch(p)]) %= MOD; }
    ...
    data_type _query(int L, int R, int type, int p, int l, int r) {
        ...
        data_type res = 0;
        if (L <= _mid(l, r)) (res += _query(L, R, type, _lch(p), l, _mid(l, r))) %= MOD;
        if (R > _mid(l, r)) (res += _query(L, R, type, _rch(p), _mid(l, r) + 1, r)) %= MOD;
        return res;
    }
    ...
    while (m--) {
        ...
        switch (op) {
            case 1:
                ...
                break;
            case 2:
                i64 x = tr.query(l, r, 3) * inv(r - l + 1) % MOD, y = tr.query(l, r, 2) * inv(r - l + 1) % MOD;
