...
#undef FUNCP1_
  constexpr friend Poly czt(Poly const &f, uint32_t c, size_t m) {
    ...
    public:
      constexpr rpow(uint32_t _base, uint32_t _mod) {
        base = _base;
        mod = _mod;
        block1[0] = block2[0] = 1;
        for (uint32_t i = 1; i < 65536; i++)
          block1[i] = (uint32_t)((uint64_t)block1[i - 1] * base % mod);
        uint32_t _ = (uint32_t)((uint64_t)block1.back() * base % mod);
        for (uint32_t i = 1; i < 65536; i++)
...
using poly = tifa_libs::poly::d_poly<0, double>;
