...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  int n, m;
  cin >> n >> m;
  vvc<int> a(n, vc<int>(n));
  {
    int cnt = 0;
    for (auto &i : a)
      for (auto &j : i) j = ++cnt;
  }
  auto clockwise = [](pii const &p) -> pii { return pii{p.second, -p.first}; };
  auto conterclockwise = [](pii const &p) -> pii {
  ...
  for_(k, 1, m, x, y, r, z) {
    ...
    --x;
    --y;
    vvc<int> b(n, vc<int>(n));
    for_(i, -r, r)
      for_(j, -r, r) {
        auto [xx, yy] = z ? conterclockwise({i, j}) : clockwise({i, j});
        b[xx + x][yy + y] = a[x + i][y + j];
      }
    for_(i, -r, r)
      for_(j, -r, r) a[x + i][y + j] = b[x + i][y + j];
  }
  for (auto &&i : a) cout << i << '\n';
}
int main() {
