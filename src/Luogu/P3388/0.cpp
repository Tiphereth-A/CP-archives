...
struct v_bcc {
  ...
  CEXP v_bcc(alist<with_deg> CR g) NE: dfn(g.size()), low(g.size()) {
    if CEXP (get_cut) sum_cut = 0, cut = vecb(g.size());
    vecu stk;
    u32 cnt = 0, start;
    auto tarjan = [&](auto &&f, u32 u, u32 fa) -> void {
      ...
      for (auto v : g[u])
        if (!dfn[v]) {
          ...
          if (low[v] >= dfn[u]) {
            if CEXP (get_cut)
              if (u != start) sum_cut += !cut[u], cut[u] = true;
            if CEXP (get_belongs) {
              vecu now;
              now.reserve(stk.size() + 1);
              while (!stk.empty() && (now.empty() || now.back() != v))
                now.push_back(stk.back()), stk.pop_back();
              now.push_back(u), now.shrink_to_fit(), belongs.push_back(now);
            }
          }
        } else if (v != fa) low[u] = min(low[u], dfn[v]);
      if CEXP (get_cut)
        if (son >= 2 && u == start) sum_cut += !cut[u], cut[u] = true;
      if CEXP (get_belongs)
        if (!~fa && !son) belongs.push_back({u});
    };
    flt_(u32, i, 0, g.size()) if (!dfn[i]) tarjan(tarjan, start = i, -1_u32);
  }
};
}  // namespace tifa_libs::graph
int main() {
  ...
  std::cout << bcc.sum_cut << '\n';
  flt_(u32, i, 0, n) if (bcc.cut[i]) std::cout << i + 1 << ' ';
  return 0;
}
