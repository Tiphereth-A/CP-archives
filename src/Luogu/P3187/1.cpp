...
struct ConvexHull2: public Polynomial2 {
    ...
    friend inline data_t max_dis_CVH(const ConvexHull2 &cvhl, const ConvexHull2 &cvhr) {
        size_t is = 0, js = 0;
        ...
        for (size_t i = 0; i < szl; ++i) {
            size_t state;
            while ((state = sign(cross_mul(cvhl[is] - cvhl[cvhl.next(is)], cvhr[js] - cvhr[cvhr.next(js)]))) < 0) js = cvhr.next(js);
    ...
    friend inline Polynomial2 coverage_rect_with_min_area(const ConvexHull2 &lhs) {
        ...
        Point2 dir = Line2{lhs[ans_i], lhs[lhs.next(ans_i)]}.direction(), vdir = rot90(dir);
    ...
    friend inline Polynomial2 coverage_rect_with_min_circum(const ConvexHull2 &lhs) {
        size_t sz = lhs.vs.size();
        if (sz == 0) return lhs;
        if (sz == 1) return Polynomial2{cont_t<Point2>{lhs[0], lhs[0], lhs[0], lhs[0]}};
        if (sz == 2) return Polynomial2{cont_t<Point2>{lhs[0], lhs[0], lhs[1], lhs[1]}};
