type bnum=array[0..250] of longint;
var a,c,x,cnt,ans1,ans2:bnum;
    s,s2:string;
    flag:boolean;
...
begin
  ...
  if not flag then for i:=a[0] downto 1 do begin
...
begin
  ...
  dec(i);
  if s2<>'' then tobig(ans1,s2);
  while (i<n) do begin
    inc(i);
    if s[i] in ['+','-'] then begin
      if s[i]='+' then bz:=1 else bz:=2;
      init(cnt);
      inc(i);
      if s[i]='(' then begin
        inc(i);
    ...
    tobig(x,s2);
    mul(x,cnt);
    if bz=1 then add(ans1,x) else add(ans2,x);
  end;
  sub(ans1,ans2);
  print(ans1);
end;
