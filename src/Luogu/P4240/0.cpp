...
using namespace std;
constexpr int64_t
qpow(int64_t a, int64_t b = MOD - 2, const int64_t &mod = MOD) {
  int64_t res(1);
  for (; b; b >>= 1, (a *= a) %= mod)
    if (b & 1) (res *= a) %= mod;
  return res;
...
int64_t phi[N], f[N];
vc<i64> g[N], h[M + OFFSET][M + OFFSET];
void init_prime(const int &n = N - 1, const int &m = M - 1) {
  phi[1] = f[1] = 1;
  for (int i = 2; i <= n; ++i) {
    if (!vis[i]) f[i] = qpow(phi[i] = (prime[++cnt_prime] = i) - 1);
  ...
  for_(t, 1, n) {
    g[t].push_back(0);
    for_(x, 1, n / t) g[t].push_back((g[t].back() + phi[x * t]) % MOD);
  }
  for_(i, 0, m) h[0][i].resize(m + 1);
  for_(i, 1, m) {
    h[i][0].resize(m + 1);
    for_(j, 1, m) {
      h[i][j].push_back(0);
      for_(k, 1, n / max(i, j))
        h[i][j].push_back((h[i][j].back() + f[k] * g[k][i] % MOD * g[k][j]) %
                          MOD);
    }
  }
}
