...
constexpr bool solve_2equ(
  int64_t b1, int64_t b2, int64_t m1, int64_t m2, int64_t &x, int64_t &M) {
  ((b1 %= m1) += m1) %= m1;
  ((b2 %= m2) += m2) %= m2;
  int64_t g = std::gcd(m1, m2), r = b2 - b1;
  M = m1 / g * m2;
  if (r % g) return false;
  auto [__, k1, k2] = exgcd(m1, m2);
  if (r < 0) k1 = -k1;
  ((k1 %= M) += M) %= M;
  k1 = mul_mod(k1, std::abs(r) / g, M);
  x = ((mul_mod(k1, m1, M) + b1) % M + M) % M;
  return true;
}
std::pair<bool, int64_t> excrt(std::vector<int64_t> const &b,
                               std::vector<int64_t> const &m) {
  assert(b.size() == m.size());
  int64_t pre_b = b[0], pre_m = m[0];
  int64_t now_b = 0, now_m = 0;
