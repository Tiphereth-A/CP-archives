...
namespace tifa_libs::math {
template <class T>
poly<T> poly_shl(poly<T> const &p, size_t x) {
  if (!x) return p;
  auto _ = p;
  if (x >= _.size()) {
...
namespace tifa_libs::math {
template <class T>
poly<T> poly_tsh(poly<T> const &f,
...
poly<T> stirling1_row_gen(u64 n, vec<u64> const &fact, vec<u64> const &ifact) {
...
template <class T, bool with_sgn = true>
poly<T> stirling1_row_gen(u64 n) {
  return stirling1_row_gen<T, with_sgn>(
    n,
...
namespace tifa_libs::math {
template <class T>
vec<T> conv_naive(vec<T> const &l, vec<T> const &r, size_t ans_size) {
...
template <class T>
vec<T> conv_naive(vec<T> const &l, vec<T> const &r) {
...
namespace tifa_libs::math {
template <class mint>
vec<mint> conv_ntt(vec<mint> l, vec<mint> r, size_t ans_size) {
  static NTT<mint> ntt;
...
template <class mint>
vec<mint> conv_ntt(vec<mint> const &l, vec<mint> const &r) {
  return conv_ntt(l, r, l.size() + r.size() - 1);
}
template <class mint>
vec<mint> conv_ntt(vec<u64> const &l, vec<u64> const &r, size_t ans_size) {
  vec<mint> l_, r_;
  l_.reserve(l.size());
  r_.reserve(r.size());
...
template <class mint>
vec<mint> conv_ntt(vec<u64> const &l, vec<u64> const &r) {
  return conv_ntt(l, r, l.size() + r.size() - 1);
}
}  // namespace tifa_libs::math
namespace tifa_libs::math {
...
int main() {
  ...
  tifa_libs::u32 n;
  std::cin >> n;
  std::cout << tifa_libs::math::stirling1_row_gen<polyd, false>(n);
  return 0;
}
