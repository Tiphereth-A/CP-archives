#include <bits/stdc++.h>
using namespace std;
template <typename Val_t = int, typename ID_t = int, const std::size_t _Edge_len = 500 + 5, const std::size_t _Node_size = 7000 + 5>
class DLX {
  private:
    struct Node {
        ID_t l, r, u, d;
...
#define _for(i, start, dir) for (std::size_t i = __##dir(start); i != start; i = __##dir(i))
    void remove_col(std::size_t _col) {
        __r(__l(_col)) = __r(_col);
        __l(__r(_col)) = __l(_col);
        _for(i, _col, d) _for(j, i, r) {
            __u(__d(j)) = __u(j);
            __d(__u(j)) = __d(j);
            --cnt_col[__col(j)];
        }
  ...
  public:
    ...
    void insert_row(std::size_t _ln, ID_t* _cols, std::size_t _len_cols) {
        for (std::size_t i = 1; i <= _len_cols; ++i) {
            node[cnt_node + i] = {cnt_node + i - 1, cnt_node + i + 1, __u(_cols[i]), _cols[i], _ln, _cols[i]};
    ...
    void insert_map(Val_t** _map, std::size_t _width, std::size_t _height, std::size_t _height_phy = _Edge_len) {
