...
struct Node final: public exEuclid::Node_base<i64> {
  using base = exEuclid::Node_base<i64>;
  i64 f;
  Node(i64 const &u = 0, i64 const &r = 0, i64 const &f = 0)
    : base(u, r), f(f) {}
  Node operator+(Node const &rhs) const {
    return Node{u + rhs.u, r + rhs.r, f + rhs.f + u * rhs.r};
  }
};
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  i64 _ = c / a;
  Node ans =
    exEuclid::qpow(u, c / b) + exEuclid::solve(b - a, b, c % b, _, u, r);
  cout << (ans.f + c / b - ((_ + 1) * _ / 2) + _ + 1) << '\n';
}
int main() {
