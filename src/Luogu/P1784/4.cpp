...
const int N = 3, N2 = N * N, N4 = N2 * N2;
int main() {
  ios::sync_with_stdio(false);
  cin.tie(nullptr);
  cout.tie(nullptr);
  for (int i = 0; i < N2; ++i)
    for (int j = 0; j < N2; ++j) cin >> sdk[i][j];
  vector<vector<bool>> maps(N2 * N4, vector<bool>(4 * N4));
  for (int i = 0; i < N2; ++i)
    for (int j = 0; j < N2; ++j) {
      for (int k = 1; k <= N2; ++k) {
        if (sdk[i][j] && sdk[i][j] != k) continue;
        auto _ = (k - 1) * N4 + i * N2 + j;
        maps[_][i * N2 + j] = maps[_][i * N2 + k + N4 - 1] =
          maps[_][j * N2 + k + N4 * 2 - 1] =
            maps[_][(i / N * N + j / N) * N2 + k + N4 * 3 - 1] = true;
      }
    }
  auto &&[flag, res] = DLX(maps).dance();
  for (auto &&_ : res) {
    auto k = (_ - 1) / (N4) + 1, i = (_ - 1) % (N4) / N2, j = (_ - 1) % N2;
    sdk[i][j] = k;
  }
  for (int i = 0; i < N2; ++i)
    for (int j = 0; j < N2; ++j) cout << sdk[i][j] << " \n"[j == N2 - 1];
  return 0;
}
