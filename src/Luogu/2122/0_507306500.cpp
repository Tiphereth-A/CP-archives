...
#define _mid(l, r) (((l) + (r)) >> 1)
    ...
    inline void _push_down(int p, i64 len_l, i64 len_r) {
        if (!tag[p]) return;
        data[_lch(p)] = {data[_lch(p)].a + tag[p] * len_l, data[_lch(p)].a_sqr + data[_lch(p)].a * tag[p] * 2 + tag[p] * tag[p] * len_l};
        data[_rch(p)] = {data[_rch(p)].a + tag[p] * len_r, data[_rch(p)].a_sqr + data[_rch(p)].a * tag[p] * 2 + tag[p] * tag[p] * len_r};
        tag[_lch(p)] += tag[p];
        tag[_rch(p)] += tag[p];
        tag[p] = 0;
    }
