...
#define SCALA_FUNC(name, func_body)        \
    ...
    SCALA_FUNC(arg2pi, { data_t tmp_= arg(); return is_negative(tmp_) ? tmp_ + 2 * PI : tmp_; })
...
#define SELF_FUNC(name, func_body)           \
    ...
    SELF_FUNC(rot90, { data_t tmp = x; x = -y; y = tmp; return *this; })
    SELF_FUNC(rot180, { x = -x; y = -y; return *this; })
    SELF_FUNC(rot270, { data_t tmp = y; y = -x; x = tmp; return *this; })
...
#undef SELF_FUNC
    ...
    friend constexpr Point2 rot(Point2 lhs, data_t theta) { return lhs.do_rot(theta); }
};
...
struct Line2 {
    ...
    inline friend bool operator==(const Line2 &lhs, const Line2 &rhs) { return lhs.l == rhs.l && lhs.r == rhs.r; }
    ...
    friend constexpr Line2 push(Line2 line, data_t dist) { return line.do_push(dist); }
};
...
struct Triangle {
    ...
    inline friend bool operator==(const Triangle &lhs, const Triangle &rhs) { return lhs.A == rhs.A && lhs.B == rhs.B && lhs.C == rhs.C; }
...
struct Circle2 {
    ...
    inline friend bool operator==(const Circle2 &lhs, const Circle2 &rhs) { return lhs.o == rhs.o && lhs.r == rhs.r; }
...
inline cont_t<Point2> ins_CL(const Circle2 &c, const Line2 &l1) {
    ...
    data_t x = dot_mul(l1.l - c.o, l1.r - l1.l), y = l1.direction().norm(), d = std::max(x * x - y * ((l1.l - c.o).norm() - c.r * c.r), data_t{});
    Point2 m = l1.l - l1.direction() * (x / y), dr = l1.direction() * (sqrt(d) / y);
    return {m - dr, m + dr};
}
...
constexpr Circle2 make_C_PPP(const Point2 &p1, const Point2 &p2, const Point2 &p3) {
...
inline cont_t<Circle2> make_C_rLL(data_t r, const Line2 &l1, const Line2 &l2) {
    ...
    dir1 *= r / dir1.abs();
    dir2 *= r / dir2.abs();
    Point2 dirl1 = rot90(dir1), dirr1 = rot270(dir1), dirl2 = rot90(dir2), dirr2 = rot270(dir2);
...
inline data_t sarea_CT(const Circle2 &c, const Point2 &p1, const Point2 &p2) {
...
constexpr data_t area_CC(const Circle2 &lhs, const Circle2 &rhs) {
    ...
    if (relation == Circle2::lying_out || relation == Circle2::touch_ex) return data_t{};
    if (relation == Circle2::lying_in || relation == Circle2::touch_in) return std::min(lhs.area(), rhs.area());
...
inline uint_data_t max_cover_Ps(const cont_t<Point2> &vp, const data_t r) {
...
struct Polynomial2 {
    ...
    explicit Polynomial2(size_t size_):
        vs(size_) {}
    explicit Polynomial2(const cont_t<Point2> &vs_):
        vs(vs_) {}
    ...
    inline Polynomial2 &resort() {
        std::sort(vs.begin(), vs.end());
        return *this;
    }
    inline void resort_in_clockwise() {
        std::sort(vs.begin(), vs.end(), [](const Point2 &lhs, const Point2 &rhs) { return lhs.quad() == rhs.quad() ? is_negative(cross_mul(lhs, rhs)) : lhs.quad() > rhs.quad(); });
    }
    inline void resort_in_counterclockwise() {
        std::sort(vs.begin(), vs.end(), [](const Point2 &lhs, const Point2 &rhs) { return lhs.quad() == rhs.quad() ? is_positive(cross_mul(lhs, rhs)) : lhs.quad() < rhs.quad(); });
    }
    inline Polynomial2 &reunique() {
        resort();
        vs.erase(std::unique(vs.begin(), vs.end()), vs.end());
        return *this;
    }
    inline Point2 &operator[](size_t x) { return vs[x]; }
    inline const Point2 &operator[](size_t x) const { return vs[x]; }
    inline cont_t<Point2>::const_iterator prev(cont_t<Point2>::const_iterator it) const { return --(it == vs.begin() ? it = vs.end() : it); }
    inline cont_t<Point2>::const_iterator next(cont_t<Point2>::const_iterator it) const { return ++it == vs.end() ? vs.begin() : it; }
    inline size_t prev(size_t idx) const { return idx == 0 ? vs.size() - 1 : idx - 1; }
    inline size_t next(size_t idx) const { return idx + 1 == vs.size() ? 0 : idx + 1; }
#define SCALA_FUNC(name, func_body)     \
    inline auto name() const func_body; \
    friend inline auto name(const Polynomial2 &lhs) { return lhs.name(); }
    SCALA_FUNC(circum, {
    ...
    SCALA_FUNC(area, {
        ...
        for (size_t i = 0; i < vs.size() - 1; ++i) ret += cross_mul(vs[i], vs[i + 1]);
        return ret / 2;
    })
#undef SCALA_FUNC
    ...
    static const size_t outside = 0;
    virtual inline size_t relation_P(const Point2 &p) const {
        for (auto &&now : vs)
    ...
    static const size_t touch_in = 1;
    static const size_t otherwise = 0;
    inline size_t relation_C(const Circle2 &c) const {
        size_t x = covered;
    ...
    inline uint_data_t cnt_intP_border() {
        uint_data_t ret = 0;
        for (size_t i = 0; i < vs.size(); ++i) ret += gcd(f2uint(std::abs(vs[i].x - vs[next(i)].x)), f2uint(std::abs(vs[i].y - vs[next(i)].y)));
        return ret;
    }
    inline uint_data_t cnt_intP_inner() { return f2uint(area()) - cnt_intP_border() / 2 + 1; }
    inline Point2 mass_point() const {
        Point2 ret{};
        data_t area{};
...
inline data_t area_PolyC(const Polynomial2 &poly, const Circle2 &c) {
    data_t ans{};
...
struct ConvexHull2: public Polynomial2 {
    ...
    explicit ConvexHull2(size_t size_):
        Polynomial2(size_) {}
    explicit ConvexHull2(const cont_t<Point2> &vs_, bool inited = false, bool strict = true):
        Polynomial2(vs_) {
    ...
    inline ConvexHull2 &init_nonstrict() {
        reunique();
        ptrdiff_t sz = vs.size();
        if (sz <= 1) return *this;
        cont_t<Point2> cvh(sz * 2);
    ...
    inline size_t relation_P(const Point2 &p) const override {
        for (auto &&now : vs)
            if (now == p) return onendpoint;
        size_t sz = vs.size();
        ...
        if (is_positive(cross(vs.front(), p, vs[1])) || is_positive(cross(vs.front(), vs.back(), p))) return outside;
        auto it = std::lower_bound(vs.begin() + 1, vs.end(), p, [&](const Point2 &lhs, const Point2 &rhs) { return is_positive(cross(vs.front(), lhs, rhs)); }) - 1;
    ...
    friend inline bool is_ins_cvh(const ConvexHull2 &cvhl, const ConvexHull2 &cvhr) {
        ...
        if (n > 1 && m > 1) {
            for (size_t l = 0, nextl = 1; l < n; nextl = cvhl.next(l = nextl))
                for (size_t r = 0, nextr = 1; r < m; nextr = cvhl.next(r = nextr))
                    if (is_ins_SS({cvhl[l], cvhl[nextl]}, {cvhr[r], cvhr[nextr]})) return false;
        } else if (n == 1) {
    ...
    inline data_t diameter() const {
        ...
        do {
            (++(cross_mul(vs[next(i)] - vs[i], vs[next(j)] - vs[j]) >= 0 ? j : i)) %= sz;
            ret = std::max(ret, dist_PP(vs[i], vs[j]));
        } while (i != is || j != js);
        return ret;
    }
    friend inline Polynomial2 coverage_rect_with_min_area(ConvexHull2 &lhs) {
        size_t sz = lhs.vs.size();
        if (sz == 0) return lhs;
        if (sz == 1) return Polynomial2{cont_t<Point2>{lhs[0], lhs[0], lhs[0], lhs[0]}};
        if (sz == 2) return Polynomial2{cont_t<Point2>{lhs[0], lhs[0], lhs[1], lhs[1]}};
        ...
        Point2 dir = Line2{lhs[ans_i], lhs[ans_i + 1]}.direction(), vdir = rot90(dir);
        Line2 li{lhs[ans_i], lhs[ans_i] + dir}, lp{lhs[ans_p], lhs[ans_p] + vdir}, lr{lhs[ans_r], lhs[ans_r] + dir}, lq{lhs[ans_q], lhs[ans_q] + vdir};
        Polynomial2 ret{cont_t<Point2>{ins_LL(li, lp), ins_LL(lp, lr), ins_LL(lr, lq), ins_LL(lq, li)}};
        lhs.vs.pop_back();
        return ret;
    }
    friend inline Polynomial2 coverage_rect_with_min_circum(ConvexHull2 &lhs) {
        size_t sz = lhs.vs.size();
        if (sz == 0) return lhs;
        if (sz == 1) return Polynomial2{cont_t<Point2>{lhs[0], lhs[0], lhs[0], lhs[0]}};
        if (sz == 2) return Polynomial2{cont_t<Point2>{lhs[0], lhs[0], lhs[1], lhs[1]}};
        ...
        for (size_t i = 0; i < sz; ++i) {
            ...
            if (i == 0) q = p;
            while (!is_positive(dot(lhs.vs[i], lhs.vs[i + 1], lhs.vs[q + 1]) - dot(lhs.vs[i], lhs.vs[i + 1], lhs.vs[q]))) q = (q + 1) % sz;
            data_t tmp = (cross(lhs.vs[i], lhs.vs[i + 1], lhs.vs[r]) + (dot(lhs.vs[i], lhs.vs[i + 1], lhs.vs[p]) - dot(lhs.vs[i], lhs.vs[i + 1], lhs.vs[q]))) / (lhs.vs[i] - lhs.vs[i + 1]).abs();
            if (ans > tmp) {
                ans = tmp;
                ans_i = i;
                ans_r = r;
    ...
    inline ConvexHull2 &do_minkowski_sum_nonstrict(const ConvexHull2 &rhs) {
        ...
        for (size_t idxl = midxl, idxr = midxr; !(idxl == midxl && fl) || !(idxr == midxr && fr);) {
            ...
            bool f = !(idxl == midxl && fl) && ((idxr == midxr && fr) || is_positive(cross_mul(diffl, diffr)));
    ...
    friend inline ConvexHull2 minkowski_sum_nonstrict(ConvexHull2 poly, const ConvexHull2 &rhs) { return poly.do_minkowski_sum_nonstrict(rhs); }
    inline ConvexHull2 &do_minkowski_sum(const ConvexHull2 &rhs) { return do_minkowski_sum_nonstrict(rhs).init(); }
    friend inline ConvexHull2 minkowski_sum(ConvexHull2 poly, const ConvexHull2 &rhs) { return poly.do_minkowski_sum(rhs); }
    inline ConvexHull2 &do_ins_CVHhP(const Line2 &l) {
        size_t sz = vs.size();
        cont_t<Point2> cvc;
        for (size_t i = 0; i < sz; ++i) {
    ...
    friend inline ConvexHull2 ins_CVHhP(ConvexHull2 poly, const Line2 &l) { return poly.do_ins_CVHhP(l); }
};
inline ConvexHull2 ins_hP(cont_t<Line2> vl) {
    auto check = [](const Line2 &u, const Line2 &v, const Line2 &w) -> bool { return w.is_include(ins_LL(u, v)); };
...
struct DynamicCVH2 {
    struct DynamicCVH2Base {
        ...
        explicit DynamicCVH2Base(ptrdiff_t sgn = 1):
            sgn(sgn) {}
        ...
        inline bool erase(cont_set_t<Point2>::const_iterator it) {
            if (it == vs.begin()) return false;
            auto j = it, k = it;
            if (++k == vs.end()) return false;
            return cross_sign(*(--j), *it, *k) * sgn >= 0 ? vs.erase(it), true : false;
        }
        inline DynamicCVH2Base &insert(const Point2 &p) {
    ...
    DynamicCVH2() = default;
    inline bool contains(const Point2 &p) const { return hcvh_up.contains(p) && hcvh_down.contains(p); }
    inline DynamicCVH2 &insert(const Point2 &p) {
        hcvh_up.insert(p);
        hcvh_down.insert(p);
        return *this;
    }
    inline ConvexHull2 to_CVH() const {
        ConvexHull2 ret;
...
inline auto solve([[maybe_unused]] int t_) -> void {
    ...
    read_var_(int, p);
    read_container_(vc<Point2>, vp, p);
    vl.emplace_back(vp[0], vp[p - 1]);
    for_(i, 0, p - 2) vl.emplace_back(vp[i + 1], vp[i]);
    sort(vl.begin() + 4, vl.end());
    data_t ans = DATA_MAX;
    Point2 INFP{1e5, 1e5};
    do {
        data_t now_ans = 0;
        ...
        for_(i, 4, vl.size() - 1) {
            ...
            debug_withname_(vl[i]);
            for_(j, 0, i - 1) {
                if (!is_ins_LL(vl[i], vl[j])) continue;
                Point2 p = ins_LL(vl[i], vl[j]);
                if (p.x < 0 || p.y < 0 || p.x > n || p.y > m) continue;
                debug_withname_(vl[j]);
                debug_withname_(p);
                if (!is_less(dist_PP(vl[i].l, p), dist_PP(vl[i].r, p)) && is_less(dist_PP(vl[i].l, p), dist_PP(vl[i].l, l))) l = p;
                if (!is_less(dist_PP(vl[i].r, p), dist_PP(vl[i].l, p)) && is_less(dist_PP(vl[i].r, p), dist_PP(vl[i].r, r))) r = p;
                debug_withname_(l);
                debug_withname_(r);
            }
            now_ans += dist_PP(l, r);
            debug_withname_(dist_PP(l, r));
        }
        chkmin(ans, now_ans);
        debug_withname_(now_ans);
        debug("---");
    } while (next_permutation(vl.begin() + 4, vl.end()));
    cout << ans;
}
int main() {
