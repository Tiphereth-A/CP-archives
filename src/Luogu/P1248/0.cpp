...
int main() {
  ...
  while (t--) {
    int n;
    scanf("%d", &n);
    _fp(i, 1, n) scanf("%lld", &minister[i].a);
    _fp(i, 1, n) scanf("%lld", &minister[i].b);
    _fp(i, 1, n) minister[i].id = i;
    _fp(i, 1, n) minister[i].index =
      (minister[i].a > minister[i].b) ?
        1 :
    ...
    printf("%lld\n", ans);
    _fp(i, 1, n) printf("%lld ", minister[i].id);
  }
  return 0;
}
