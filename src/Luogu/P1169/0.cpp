...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  for_(i, 1, n)
    for_(j, 1, m) cin >> a[i][j];
  vvc<int> l(n + 1, vc<int>(m + 1)), r(n + 1, vc<int>(m + 1)),
    b(n + 1, vc<int>(m + 1, 1));
  for (auto &i : l) iota(i.begin(), i.end(), 0);
  for (auto &i : r) iota(i.begin(), i.end(), 0);
  for_(i, 1, n)
    for_(j, 2, m)
  ...
  for_(i, 1, n)
    for_(j, 1, m) {
      ans1 = max(ans1, min(r[i][j] - l[i][j] + 1, b[i][j]));
      ans2 = max(ans2, (r[i][j] - l[i][j] + 1) * b[i][j]);
    }
  cout << ans1 * ans1 << '\n' << ans2 << '\n';
}
int main() {
