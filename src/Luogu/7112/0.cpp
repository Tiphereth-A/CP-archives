        ...
        _for(i, 0, now.get_row(), now_row = 0) {
            ...
            _for(j, i + 1, now.get_row()) {
                ...
                while (now.data(j, i)) {
                    ...
                    _for(k, i, now.get_col()) now.data(i, k) -= now.data(j, k) * data_t(_, now.data(0, 0).get_mod());
                    std::swap(now.mat[j], now.mat[i]);
                    neg ^= true;
                }
            }
        }
        return neg ? -static_cast<std::ptrdiff_t>(now.get_row()) : static_cast<std::ptrdiff_t>(now.get_row());
    }
    inline constexpr data_t det() {
        std::ptrdiff_t rk = gauss_half(*this);
        if (rk == 0) return data_t(0, this->data(0, 0).get_mod());
        data_t ans(rk < 0 ? this->data(0, 0).get_mod() - 1 : 1, this->data(0, 0).get_mod());
