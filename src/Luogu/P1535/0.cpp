...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  p2 -= {1, 1};
  int ans = 0;
  auto f = [&](auto &&f, pii const &now, int now_t) -> void {
    if (now == p2 && !now_t) ++ans;
    if (!now_t) return;
    if (pii _ = now - p2; abs(_.first) + abs(_.second) > now_t) return;
    for (auto &&d : DIR4)
      if (pii _ = d + now; _.first >= 0 && _.first < n && _.second >= 0 &&
                           _.second < m && mp[_.first][_.second] == '.')
        f(f, _, now_t - 1);
  };
  f(f, p1, t);
  cout << ans << '\n';
