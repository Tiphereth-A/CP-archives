...
std::optional<data_t> quad_residue(data_t n, data_t p) {
...
template <size_t DEG_LIMIT, data_t MOD>
class NTT_ {
  constexpr static data_t G = proot<MOD>, IG = inverse(G, MOD);
...
protected:
  ...
  template <class F1, class Fodd, class Feven>
  void expand_base__(
    Poly &ans, size_t n, F1 &&f1, Fodd &&fodd, Feven &&feven) const {
    if (n == 1) {
      ans.data.push_back(f1());
      return;
    }
    if (n & 1) {
      expand_base__(ans, n - 1, f1, fodd, feven);
      ans.data.push_back(fodd(ans, n));
      return;
    }
    expand_base__(ans, n / 2, f1, fodd, feven);
    feven(ans, n);
  }
  void inv_(Poly &ans, size_t n) const {
    expand_base__(
      ans,
      n,
      [this]() -> data_t { return (data_t)inverse(data[0], MOD); },
      [this](Poly &ans, size_t &n) -> data_t {
        --n;
        ldata_t _ = 0;
        for (size_t i = 0; i < n; ++i)
          _ = (_ + (ldata_t)ans[i] * data[n - i]) % MOD;
        return (data_t)(_ * inverse(MOD - data[0], MOD) % MOD);
      },
  ...
  void exp_(Poly &ans, size_t n) const {
    expand_base__(
      ans,
      n,
      [this]() -> data_t { return 1; },
      [this](Poly &ans, size_t &n) -> data_t {
        n -= 2;
        ldata_t _ = 0;
        for (size_t i = 0; i <= n; ++i)
          _ = (_ + (i + 1) * data[i + 1] % MOD * ans[n - i] % MOD) % MOD;
        return (data_t)(_ * Helper::inv[n + 1] % MOD);
      },
      [this](Poly &ans, size_t n) -> void {
        Poly ans_log = ans;
        ans_log.resize(n);
