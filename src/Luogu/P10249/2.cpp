...
public:
  ...
  void rebind(FILE *f = nullptr) {
    ...
    auto fd = fileno(f_ = f);
    fstat(fd, &Fl),
      p = (bg = (char *)mmap(0, Fl.st_size + 4, PROT_READ, MAP_PRIVATE, fd, 0)),
      ed = bg + Fl.st_size, madvise(bg, Fl.st_size + 4, MADV_SEQUENTIAL);
  }
  char peek() { return *p; }
  char get() { return *p++; }
...
public:
  ...
  fastout &write(const char *n) {
    ...
    const char *n_ = n;
    while (p + len >= ed)
      memcpy(p, n_, l_ = usz(ed - p)), p += l_, n_ += l_, len -= l_, flush();
  ...
  fastout &write(T n) {
    if CEXP (sizeof(T) <= 4)
      return memset(p_ib = int_buf, 0, 11), u32tostr(n, p_ib), write(p_ib);
...
public:
  ...
  CEXPE poly(u32 sz = 1, cT_(val_t) val = val_t{}): d(sz, val) {}
  CEXP poly(TPN data_t::const_iterator begin, TPN data_t::const_iterator end)
    : d(begin, end) {}
  CEXP poly(itl<val_t> v): d(v) {}
  CEXP poly(spn<val_t> v): d(v.begin(), v.end()) {}
  ...
  CEXP val_t &operator[](u32 x) { return d[x]; }
  CEXP val_t CR operator[](u32 x) const { return d[x]; }
  ...
  CEXP void strip() {
    ...
    if (d.resize(usz(d.rend() - it)); d.empty()) d.push_back(val_t(0));
  }
...
CEXP vec<T> conv_naive(vec<U> CR l, vec<U> CR r, u32 ans_size = 0) {
  ...
  if (n < m) swap(n, m);
  flt_(u32, i, 0, n) flt_(u32, j, 0, min(m, ans_size - i)) ans[i + j] +=
    (T)l_[i] * (T)r_[j];
  return ans;
}
}  // namespace tifa_libs::math
...
CEXP vec<mint>
conv_dft_um(DFT_t &dft, vec<T> CR l, vec<T> CR r, u32 ans_size = 0) {
  if (!ans_size) ans_size = u32(l.size() + r.size() - 1);
  vec<mint> l_, r_;
...
public:
  ...
  void reset_seed() {
    re.seed((res_t)std::chrono::duration_cast<
              std::conditional_t<sizeof(res_t) <= 4,
                                 std::chrono::seconds,
...
CEXP u64 proot(u64 m) {
  if (m == 2) return 1;
  if (m == 3 || m == 5) return 2;
  if (m == 104857601 || m == 167772161 || m == 469762049 || m == 998244353 ||
      m == 1004535809)
    return 3;
...
public:
  CEXPE NTT(): sz() {
    ...
    mint prod(1), iprod(1);
    flt_(u32, i, 0, R - 1) rate[i] = prod * root[i + 2],
