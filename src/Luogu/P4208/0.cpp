    ...
    for (size_t i = row_start, now_row; i < std::min(row_end, this->col_size());
      ...
      for (int64_t j = clear_all ? 0 : rk; j < (int64_t)row_end; ++j) {
        ...
        while (true) {
          ...
          for_(x, i, this->col_size() - 1)
            (((*this)(j, x) %= MOD) += MOD) %= MOD;
...
public:
  ...
  size_t find_nopc(size_t x) const {
    return x == fa[x] ? fa[x] : find_nopc(fa[x]);
  }
  bool merge_nopc(size_t x, size_t y) {
    size_t fx = find_nopc(x), fy = find_nopc(y);
    return fx == fy ? false : (fa[fx] = fy, true);
  }
};
...
public:
  ...
  explicit KruskalSolver(size_t _n): edges(), vertex_size(_n) {
    assert(_n > 0);
  }
...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  int n, m;
  cin >> n >> m;
  read_container_(vc<pi3i>, edges, m);
  KruskalSolver<int> mst(n + 1);
  for (auto &&[u, v, w] : edges) mst.addedge(u, v, w);
  if (mst.run().data().size() < n - 1) run_return_void_(cout << "0\n");
  i64 ans = 1;
  set<int> ws;
  for (auto &&[_, __, w] : edges) ws.insert(w);
  for (auto &&i : ws) {
    DsuBasic dsu(n + 1);
    for (auto &&[w, from, to] : mst.data())
      if (w != i) dsu.merge(from, to);
    vc<int> id(n + 1);
    int cnt = 0;
    for_(i, 1, n)
      if (dsu.find(i) == i) id[i] = cnt++;
    if (cnt < 2) continue;
    for_(i, 1, n) id[i] = id[dsu.find(i)];
    matrix_int<i64> mat(cnt, cnt);
    ...
    mat =
      matrix_int<i64>(cnt - 1, cnt - 1, mat.submatrix(1, cnt, 1, cnt).view());
    mat += MOD;
    mat %= MOD;
    auto ret = mat.do_gauss(false);
    if (ret < 0) ans = MOD - ans;
    for_(i, 0, cnt - 2) (ans *= mat(i, i)) %= MOD;
  }
  cout << ans << '\n';
}
int main() {
