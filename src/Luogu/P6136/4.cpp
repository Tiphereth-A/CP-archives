...
int main() {
  ...
  for (auto i : io.a) bt.insert(i);
  for (int i = 0, opt, x; (size_t)i < m; ++i) {
    opt = io.opt(), x = io.x();
    switch (opt) {
      case 1: bt.insert(x); break;
      case 2: bt.erase(x); break;
      case 3: io.print((int)bt.order_of_key(x) + 1); break;
      case 4: io.print(bt.find_by_order((uint32_t)x - 1)->data); break;
      case 6: io.print(bt.upper_bound(x)->data); break;
      case 5:
        auto it = bt.lower_bound(x);
        io.print((it ? bt.prev(it) : bt.rightmost(bt.root))->data);
        break;
    }
  }
  io.print_total_ans();
