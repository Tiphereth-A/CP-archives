...
CEXP poly<mint, ccore> exp_fps(poly<mint, ccore> p, u32 n = 0) {
  ...
  poly<mint, ccore> a{1};
  for (u32 i = 1; i < n; i *= 2)
    a = (a * (p.pre(i * 2) - ln_fps(a, i * 2))).pre(i * 2);
  return a.pre(n);
}
}  // namespace tifa_libs::math
namespace tifa_libs::math {
template <class mint, class ccore>
CEXP poly<mint, ccore>
powem_fps(poly<mint, ccore> CR f, poly<mint, ccore> g = {1}, u32 m = 0) {
  ...
  while (n) {
    ...
    flt_(u32, i, 0, k) std::copy(
      p.begin() + i * h, p.begin() + i * h + n + 1, pp.begin() + i * h * 2);
    flt_(u32, i, 0, k) std::copy(
      q.begin() + i * h, q.begin() + i * h + n + 1, qq.begin() + i * h * 2);
    qq[k * h * 2] += 1, core.bzr(k * h * 4), core.dif(pp), core.dif(qq);
    for (u32 i = 0; i < k * h * 4; i += 2) swap(qq[i], qq[i + 1]);
    flt_(u32, i, 0, k * h * 4) pp[i] *= qq[i];
    flt_(u32, i, 0, k * h * 2) rr[i] = qq[i * 2] * qq[i * 2 + 1];
    core.dit(pp), core.bzr(k * h * 2), core.dit(rr), rr[0] -= 1,
      p.assign(k * h, 0), q.assign(k * h, 0);
    flt_(u32, i, 0, k * 2) fle_(u32, j, 0, n / 2)
      p[i * h / 2 + j] = pp[i * h * 2 + j * 2 + n % 2],
                    q[i * h / 2 + j] = rr[i * h + j];
    n /= 2, h /= 2, k *= 2;
  }
  poly_t s{p.begin(), p.begin() + k}, t{q.begin(), q.begin() + k};
  return t.data().push_back(1), s.reverse(), t.reverse(),
         (s * inv_fps(t, m + 1)).pre(m + 1);
}
}  // namespace tifa_libs::math
namespace tifa_libs::math {
template <class mint, class ccore>
CEXP poly<mint, ccore> shl_fps(poly<mint, ccore> CR p, usz x) {
  ...
  if (auto _ = p; x >= _.size()) return std::ranges::fill(_.data(), 0), _;
    ...
    return std::fill(_.data().begin(),
                     std::move_backward(_.data().begin(),
                                        std::prev(_.data().end(), (isz)x),
                                        _.data().end()),
                     0),
           _;
}
...
template <class mint, class ccore>
CEXP poly<mint, ccore>
compinv_fps(poly<mint, ccore> CR f, spnuu inv, u32 n = 0) {
...
template <class mint, class ccore>
CEXP poly<mint, ccore> compinv_fps(poly<mint, ccore> CR f, u32 n = 0) {
  return compinv_fps(f, gen_inv(f.size(), mint::mod()), n);
}
}  // namespace tifa_libs::math
