...
#undef SCALA_FUNC
    ...
    inline void resort_in_clockwise() {
        std::sort(vs.begin(), vs.end(), [](const Point2 &lhs, const Point2 &rhs) { return lhs.quad() > rhs.quad(); });
    }
...
struct Node {
    ...
    friend bool operator<(const Node &lhs, const Node &rhs) { return lhs.line < rhs.line; }
};
