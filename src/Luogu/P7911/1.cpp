...
using namespace std;
regex RE_IP(
  R"((?:(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d?|0)\.){3}(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d?|0):(?:0|6553[0-5]|655[0-2]\d|65[0-4]\d\d|6[0-4]\d{3}|[1-5]\d{4}|[1-9]\d{0,3}))");
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  for_(i, 1, n) {
    string tp, ip;
    cin >> tp >> ip;
    if (!regex_match(ip, RE_IP)) run_continue_(cout << "ERR\n");
    if (tp.front() == 'S') {
      if (auto &_ = dict[ip]; _)
        run_continue_(cout << "FAIL\n") else run_continue_(_ = i; cout << "OK"
                                                                       << '\n');
    } else {
      if (auto _ = dict[ip]; !_)
        run_continue_(cout << "FAIL\n") else run_continue_(cout << _ << '\n');
    }
  }
}
int main() {
