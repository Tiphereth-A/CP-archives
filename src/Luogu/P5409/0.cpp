...
namespace tifa_libs::math {
template <class T>
poly<T> poly_inv(poly<T> const &p, size_t n = 0) {
  assert(p[0] != 0);
  if (!n) n = p.size();
  poly<T> a{p[0].inv()};
...
namespace tifa_libs::math {
template <class T>
poly<T> poly_ln(poly<T> const &p, size_t n = 0) {
  assert(p[0] == 1);
  if (!n) n = p.size();
...
namespace tifa_libs::math {
template <class T>
poly<T> poly_exp(poly<T> const &p, size_t n = 0) {
  assert(p[0] == 0);
  if (!n) n = p.size();
  poly<T> _ = p + 1, a{1};
  for (size_t i = 1; i < n; i *= 2)
...
namespace tifa_libs::math {
template <class T>
poly<T> poly_shr(poly<T> const &p, size_t x) {
  if (!x) return p;
  auto _ = p;
  if (x >= p.size()) {
    std::fill(_.data().begin(), _.data().end(), 0);
...
namespace tifa_libs::math {
template <class T>
poly<T> poly_pow(poly<T> const &p, u64 y, size_t n = 0) {
  using mint = typename T::value_type;
  if (!n) n = p.size();
  if (y == 0) {
    poly<T> _(n);
...
template <class T, bool with_sgn = true>
poly<T> stirling1_col_gen(u64 n, u64 k) {
...
namespace tifa_libs::math {
template <class mint>
vec<mint> conv_ntt32(vec<mint> l, vec<mint> r, size_t ans_size) {
...
vec<mint> conv_ntt32(vec<T> const &l, vec<T> const &r, size_t ans_size) {
