...
protected:
  ...
  double asr(double l, double r, double area, double eps, Func &&func) const {
    double mid = r - (r - l) * .5;
    double lmid = mid - (mid - l) * .5, rmid = r - (r - mid) * .5;
    double fl = func(l), flmid = func(lmid), fmid = func(mid),
           frmid = func(rmid), fr = func(r);
    ...
    return asr(l, mid, ls, eps * .5, func) + asr(mid, r, rs, eps * .5, func);
  }

public:
  SimpsonSolver(double eps): eps__(eps) {}
  template <typename Func>
  double operator()(double l, double r, Func &&func) const {
    return asr(l,
               r,
               simpson(l, r, func(l), func(r - (r - l) * .5), func(r)),
               eps__,
               func);
  }
} simpson_solver(EPS);
...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  cin >> n;
  vc<Node> v(n);
  for (auto &[x, y, m] : v) cin >> x >> y >> m;
  vc<pair<double, double>> seg;
  seg.reserve(n);
  auto f = [&v, &seg](double pos) -> double {
    seg.clear();
    for (auto [x, y, m] : v)
  ...
  double ans = 0;
  for_(i, 1, pos.size() - 1)
    ans += simpson_solver(pos[i - 1] + EPS, pos[i] - EPS, f);
