...
auto solve([[maybe_unused]] int t_ = 0) -> void {
  ...
  cin >> n;
  invf[0] = 1;
  for (int i = 1; i <= n; ++i) invf[i] = invf[i - 1] * inv[i] % MOD;
  Poly f(n + 1), g(n + 1);
  f[0] = 1;
  for (int64_t i = 1, _pow; i <= n; ++i) {
    _pow = qpow(2, i * (i - 1) / 2 % (MOD - 1), MOD);
    f[i] = _pow * invf[i] % MOD;
    g[i] = _pow * invf[i - 1] % MOD;
  }
  cout << (g * f.do_inverse())[n] * qpow(invf[n - 1], MOD - 2, MOD) % MOD;
}
int main() {
