...
def clean_redundant_code_cpp(code: list[str]) -> list[str]:
    ...
    # remove `inline` and `register`
    code = [column.replace('inline', '').replace('register', '')
            for column in code]
    ...
    defined_flag: set[str] = {
        'ONLINE_JUDGE', f"ALWAYS_TRUE_{rand_token}", "DISABLE_PRAGMA_", "__GNUG__"}
